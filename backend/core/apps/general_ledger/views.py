...
from .serializers import (
    ...
    GLStJnlSerializer, GLStJnlListSerializer, GLStJnlDetailSerializer,
    GLSpreadSerializer, GLSpreadListSerializer, GLSpreadDetailSerializer
)
...
class GLMastViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def account_history(self, request, pk=None):
        ...
        for i in range(1, 14):
            ...
            periods.append({
                'period': i,
                'balance': float(getattr(account, field_name))
            })
        ...
        for i in range(1, 13):
            ...
            budgets.append({
                'period': i,
                'budget': float(getattr(account, field_name))
            })
        ...
        for i in range(1, 13):
            ...
            last_year_data.append({
                'period': i,
                'last_year': float(getattr(account, field_name))
            })
...
class GLTranViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def batch_summary(self, request):
        ...
        return Response({
            ...
            'date_range': {
                'earliest': transactions.aggregate(min_date=Min('date'))['min_date'],
...
class GLSpreadViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    search_fields = ['accno', 'name']
    ordering_fields = ['accno', 'name', 'ytddebit', 'ytdcredit', 'curdebit', 'curcredit']
    ordering = ['accno']
    ...
    def summary(self, request):
        ...
        total_ytd_debit = spreads.aggregate(total=Sum('ytddebit'))['total'] or Decimal('0.00')
        total_ytd_credit = spreads.aggregate(total=Sum('ytdcredit'))['total'] or Decimal('0.00')
        total_cur_debit = spreads.aggregate(total=Sum('curdebit'))['total'] or Decimal('0.00')
        total_cur_credit = spreads.aggregate(total=Sum('curcredit'))['total'] or Decimal('0.00')
        total_curbuddeb = spreads.aggregate(total=Sum('curbuddeb'))['total'] or Decimal('0.00')
        total_curbudcred = spreads.aggregate(total=Sum('curbudcred'))['total'] or Decimal('0.00')
        total_ytdbuddeb = spreads.aggregate(total=Sum('ytdbuddeb'))['total'] or Decimal('0.00')
        total_ytdbudcred = spreads.aggregate(total=Sum('ytdbudcred'))['total'] or Decimal('0.00')
