...
def validate_vat_total_calculation(value_excl_vat: Decimal, tax_amount: Decimal, total_incl_vat: Decimal):
    """
...
def validate_category_exists(category_id: int):
    ...
    expense_exists = ExpenseCategory.objects.filter(id=category_id, is_active=True).exists()
    income_exists = IncomeCategory.objects.filter(id=category_id, is_active=True).exists()
    
    if not (expense_exists or income_exists):
        raise ValidationError(
            f'Category {category_id} does not exist or is not active'
        )
...
def validate_monthly_balance_consistency(
    ...
    """
    months = [
        balance_month_01, balance_month_02, balance_month_03, balance_month_04,
        balance_month_05, balance_month_06, balance_month_07, balance_month_08,
        balance_month_09, balance_month_10, balance_month_11, balance_month_12,
    ...
    for month_num, balance in enumerate(months, start=1):
        if balance is not None and balance < Decimal('0'):
            ...
            raise ValidationError(
                f'{month_name} balance cannot be negative: {balance}'
            )
