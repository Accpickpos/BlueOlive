...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--confirm", action="store_true", help="Proceed without confirmation prompt"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["month"]:
            try:
                archive_date = timezone.datetime.strptime(
                    options["month"], "%Y-%m"
                ).date()
                # Use last day of the month
                next_month = archive_date.replace(day=1) + timedelta(days=32)
                archive_date = (next_month - timedelta(days=next_month.day)).replace(
                    day=1
                )
            except ValueError:
        ...
        transactions = CashBookTransaction.objects.filter(
            transaction_date__lt=cutoff_date, is_archived=False
        )
        ...
        updated = transactions.update(is_archived=True, archive_month=archive_date)
