...
TYPE_MAP = {
    'I': 'RECEIPT', 'R': 'RECEIPT', 'P': 'PAYMENT', 'D': 'DEPOSIT',
    'W': 'WITHDRAWAL', 'T': 'TRANSFER', 'B': 'BANK_CHARGE',
}
...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_transactions(self):
        ...
        for record in open_dbf(path):
            ...
            transaction_type = translate_code(
                self, record.get('TYPE'), TYPE_MAP, 'RECEIPT', context=f'cbtran.dbf TYPE (txn {transaction_number})'
            )
            ...
            try:
                _, created = CashBookTransaction.objects.using(self.db_alias).update_or_create(
                    ...
                    defaults={
                        ...
                        'bank_recon_tag': translate_code(
                            self, record.get('CBTAG'), TAG_MAP, 'U', context=f'cbtran.dbf CBTAG (txn {transaction_number})'
                        ),
                        'reference': reference,
                        'description': reference or f'{transaction_type} {transaction_number}',
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ CB txn {transaction_number}: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS('  ✓ Cashbook transactions imported from cbtran.dbf'))
    ...
    def _import_cheques(self):
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = UnpresentedCheque.objects.using(self.db_alias).update_or_create(
                    ...
                    defaults={
                        ...
                        'total': total,
                        'tag': translate_code(
                            self, record.get('CBTAG'), TAG_MAP, 'U', context=f'cbcheq.dbf CBTAG (cheque {cheque_number})'
                        ),
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ Cheque {cheque_number}: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS('  ✓ Unpresented cheques imported from cbcheq.dbf'))
