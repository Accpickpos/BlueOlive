...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_expense(self):
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = ExpenseCategoryBalance.objects.using(
                    self.db_alias
                ).update_or_create(expense_category=category, defaults=defaults)
                self.created += created
                self.updated += not created
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ expense category {category.number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Expense category balances imported from cbexp.dbf")
        )
    ...
    def _import_income(self):
        ...
        categories = {
            c.number: c for c in IncomeCategory.objects.using(self.db_alias).all()
        }
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = IncomeCategoryBalance.objects.using(
                    self.db_alias
                ).update_or_create(income_category=category, defaults=defaults)
                self.created += created
                self.updated += not created
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ income category {category.number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Income category balances imported from cbinc.dbf")
        )
