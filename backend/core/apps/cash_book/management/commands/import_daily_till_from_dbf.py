...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        extra_paths = [p for p in (self.pdf_dir / 'cfile.dbf', self.pdf_dir / '5cfile.dbf') if p.exists()]
        ...
        for path in extra_paths:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ {path.name}: could not derive trading date, skipping'))
                self.skipped += 1
                continue
            self._import_file(path, trading_date)
    ...
    def _import_file(self, path, trading_date):
        for record in open_dbf(path):
            ...
            try:
                _, created = DailyTillTransaction.objects.using(self.db_alias).update_or_create(
                    trading_date=trading_date, transaction_number=transaction_number, source=source,
                    defaults={
                        ...
                        'reconciliation_account_total': to_decimal(record.get('RECONACC')),
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ {path.name} ({trading_date}): {e}'))
                self.errors += 1
