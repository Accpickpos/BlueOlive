...
class CashBookTransaction(VATMixin, ReconciliationMixin, models.Model):
    ...
    account_type = models.CharField(
        max_length=10, choices=ACCOUNT_TYPE_CHOICES, default="CASH"
    )
    ...
    amount = models.DecimalField(
        max_digits=15, decimal_places=2, validators=[validate_positive_amount]
    )

    # Category & Audit Fields (per spec)
    category_id = models.IntegerField(
        blank=True, null=True, help_text="Category number from spec CATNO"
    )
    ...
    bank_recon_tag = models.CharField(
        max_length=1,
        default="U",
        choices=[
            ("R", "Reconciled"),
    ...
    running_balance_cash = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
    running_balance_bank = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
...
class OtherIncome(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="other_income"
    )
    ...
    # Payment method
    paid_into = models.CharField(
        max_length=20,
        choices=[("CASH", "Cash Till"), ("BANK", "Bank Account")],
        default="CASH",
    )
...
class OtherExpense(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="other_expense"
    )

    # Expense Details
    expense_category = models.ForeignKey(
        "settings.ExpenseCategory", on_delete=models.PROTECT
    )
...
class BankDeposit(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="bank_deposit"
    )
...
class CashWithdrawal(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="cash_withdrawal"
    )
...
class BankTransfer(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="bank_transfer"
    )
...
class BankCharge(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="bank_charge"
    )
...
class InterestReceived(models.Model):
    ...
    transaction = models.OneToOneField(
        CashBookTransaction, on_delete=models.CASCADE, related_name="interest_received"
    )
...
class BankReconciliation(models.Model):
    ...
    outstanding_cheques = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
    ...
    def is_balanced(self):
        ...
        return abs(calculated_balance - self.closing_balance_per_books) < Decimal(
            "0.01"
        )
...
class ExpenseCategoryBalance(models.Model):
    ...
    expense_category = models.OneToOneField(
        ExpenseCategory, on_delete=models.CASCADE, related_name="balance"
    )

    # Current Month To Date
    balance_month_to_date = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
    input_vat_month_to_date = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
    ...
    def year_to_date_balance(self):
        ...
        return (
            self.balance_month_01
            + self.balance_month_02
            + self.balance_month_03
            + self.balance_month_04
            + self.balance_month_05
            + self.balance_month_06
            + self.balance_month_07
            + self.balance_month_08
            + self.balance_month_09
            + self.balance_month_10
            + self.balance_month_11
            + self.balance_month_12
        )
...
class IncomeCategoryBalance(models.Model):
    ...
    income_category = models.OneToOneField(
        IncomeCategory, on_delete=models.CASCADE, related_name="balance"
    )
    ...
    output_vat_month_to_date = models.DecimalField(
        max_digits=15, decimal_places=2, default=0
    )
...
class UnpresentedCheque(models.Model):
    ...
    cheque_date = models.DateField(
        db_index=True, help_text="Transaction date per spec DATE"
    )
    reference = models.CharField(
        max_length=20, blank=True, help_text="Additional ref per spec CBREF"
    )

    # Amounts (per spec: VALUE, TAX, TOTAL)
    value = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Value excluding VAT per spec VALUE"
    )
    tax_code = models.IntegerField(
        default=1, help_text="Tax code per spec TAX (field size 10,2)"
    )
    total = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Total including VAT per spec TOTAL"
    )
    ...
    is_presented = models.BooleanField(
        default=False, db_index=True, help_text="Has cheque been presented at bank"
    )
    presented_date = models.DateField(
        null=True, blank=True, help_text="Date cheque appeared on bank statement"
    )

    # Month-end tracking (per spec: relates to reconciliation period)
    month_end_date = models.DateField(
        db_index=True, help_text="Month-end date this cheque relates to"
    )

    # Aging tracking
    days_outstanding = models.IntegerField(
        default=0, help_text="Days since cheque date"
    )
    is_stale = models.BooleanField(
        default=False, db_index=True, help_text="True if >6 months old (stale cheque)"
    )
    requires_follow_up = models.BooleanField(
        default=False, help_text="True if >3 months outstanding"
    )

    # Notes for dispute/resolution
    notes = models.TextField(
        blank=True, help_text="Dispute notes or resolution information"
    )
    ...
    def clean(self):
        ...
        if self.month_end_date < self.cheque_date:
            raise ValidationError("Month-end date must be on or after cheque date")
    ...
    def get_unpresented_summary(cls, as_of_date=None):
        ...
        unpresented = cls.objects.filter(
            is_presented=False, cheque_date__lte=as_of_date
        )

        return {
            "count": unpresented.count(),
            "total_value": unpresented.aggregate(total=models.Sum("value"))["total"]
            or Decimal("0"),
            "by_age": {
                "stale": unpresented.filter(is_stale=True).count(),
                "follow_up": unpresented.filter(requires_follow_up=True).count(),
                "current": unpresented.filter(days_outstanding__lte=90).count(),
            },
        }
...
class DailyTillTransaction(models.Model):
    ...
    cash_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="CASHDAY"
    )
    # CHEQDAY N 12,2
    cheque_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="CHEQDAY"
    )
    # CARDDAY N 12,2
    card_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="CARDDAY"
    )
    # INVTOT N 12,2
    invoice_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="INVTOT"
    )
    # CNOTETOT N 12,2
    credit_note_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="CNOTETOT"
    )
    # RECONACC N 12,2
    reconciliation_account_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="RECONACC"
    )
    # OUTVAT N 12,2
    output_vat = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="OUTVAT"
    )
    # INVAT N 12,2
    input_vat = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="INVAT"
    )
    # TIME C 5
    ...
    change_given = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="CHANGE"
    )
    # LAYBYE N 12,2
    laybye_amount = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="LAYBYE"
    )
    # COMMENTS C 75
    comments = models.CharField(max_length=75, blank=True, help_text="COMMENTS")
    # SPEEDPOINT N 12,2
    speedpoint_total = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="SPEEDPOINT"
    )
    # ADJUST N 10,2
    adjustment = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text="ADJUST"
    )
    # USER C 25
