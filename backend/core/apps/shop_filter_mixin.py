...
class ShopFilterMixin:
    ...
    def get_queryset(self):
        ...
        logger.debug(
            f"[ShopFilterMixin] User {user.id}, role={user.role}, is_superuser={user.is_superuser}"
        )

        # If user is superuser or admin, return all data
        if user.is_superuser or user.role == "ADMIN":
            logger.debug(
                f"[ShopFilterMixin] User {user.id} is admin - returning all data"
            )
            return queryset
        ...
        logger.debug(
            f"[ShopFilterMixin] User {user.id} current_shop_id (from user obj) = {shop_id}"
        )
        ...
        if not shop_id:
            ...
            logger.debug(
                f"[ShopFilterMixin] User {user.id} current_shop_id (from session) = {shop_id}"
            )
        ...
        if not shop_id:
            ...
            if shop_ids_list:
                ...
                logger.debug(
                    f"[ShopFilterMixin] User {user.id} using first assigned shop: {shop_id}"
                )
        ...
        if not shop_id:
            ...
            logger.warning(
                f"User {user.id} has no current_shop_id set, returning empty queryset"
            )
        ...
        if not user.can_access_shop(shop_id):
            logger.warning(
                f"User {user.id} cannot access shop {shop_id}, returning empty queryset"
            )
            return queryset.none()

        logger.debug(
            f"[ShopFilterMixin] User {user.id} filtering by shop_id = {shop_id}"
        )
    ...
    def _filter_by_shop(self, queryset, shop_id):
        ...
        if model_name in non_shop_models:
            logger.debug(
                f"[ShopFilterMixin] Model {model_name} has no shop_id - returning all (tenant-level data)"
            )
            return queryset
        ...
        if settings.DEBUG:
            logger.debug(
                f"[ShopFilterMixin] Model {queryset.model.__name__} has no shop_id field - returning all data"
            )
    ...
    def perform_create(self, serializer):
        ...
        if hasattr(serializer, "Meta") and hasattr(serializer.Meta, "model"):
            ...
            logger.debug(
                f"[ShopFilterMixin] perform_create: Serializer {class_name} has no Meta - checking class name"
            )
        ...
        logger.debug(
            f"[ShopFilterMixin] perform_create: Identified model_name={model_name}"
        )

        # DIAGNOSTIC: Log all fields in validated_data before processing
        if settings.DEBUG:
            logger.debug(
                f"[ShopFilterMixin] perform_create: validated_data keys = {list(serializer.validated_data.keys())}"
            )
        ...
        if model_name and model_name in non_shop_models:
            if "shop_id" in serializer.validated_data:
                logger.debug(
                    f"[ShopFilterMixin] perform_create: Removing shop_id for {model_name}"
                )
            ...
            if hasattr(self.request, "user") and self.request.user.is_authenticated:
                ...
                if not shop_id:
                    ...
                    if shop_ids_list:
                        shop_id = shop_ids_list[0]
                        logger.debug(
                            f"[ShopFilterMixin] perform_create: Using first assigned shop {shop_id}"
                        )
                ...
                if shop_id:
                    ...
                    if "shop_id" not in serializer.validated_data:
                        if "shop" not in serializer.validated_data:
                            ...
                            if settings.DEBUG:
                                logger.debug(
                                    f"Auto-assigned shop_id={shop_id} on create for {model_name or 'unknown model'}"
                                )
