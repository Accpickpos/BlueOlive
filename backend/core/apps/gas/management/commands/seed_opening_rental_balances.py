...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
            ...
            for lineno, row in enumerate(reader, start=2):
                try:
                    ...
                    stock_item = StockItem.objects.get(
                        stock_code=row["stock_item_code"]
                    )
                    quantity = Decimal(row["quantity"])
                    deposit_amount = Decimal(row["deposit_amount"])
                except Debtor.DoesNotExist:
                    raise CommandError(
                        f"Line {lineno}: no debtor with dno={row['debtor_dno']}"
                    )
                except StockItem.DoesNotExist:
                    raise CommandError(
                        f"Line {lineno}: no stock item with code={row['stock_item_code']}"
                    )
        ...
        self.stdout.write(
            f"Parsed {len(rows)} opening rentals, total deposit liability: {total}"
        )
        ...
        with transaction.atomic():
            ...
            for debtor, stock_item, quantity, deposit_amount in rows:
                RentalTransaction.objects.create(
                    ...
                    checkout_date=today,
                    due_date=today
                    + timezone.timedelta(days=settings_row.overdue_threshold_days),
                    notes="Opening balance — seeded from cutover physical count, not a new checkout.",
                )

            account = GLMast.objects.select_for_update().get(
                accno=settings_row.deposits_held_accno
            )
