...
class RentalTransactionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def checkout(self, request):
        ...
        try:
            ...
            return Response({'error': 'Stock item not found'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            rental = RentalService.checkout_cylinder(
                debtor=debtor, stock_item=stock_item, quantity=data['quantity'],
                deposit_amount=data['deposit_amount'], user=request.user,
    ...
    def returned(self, request, pk=None):
        ...
        try:
            rental = RentalService.return_cylinder(
                rental_id=pk, reconciliation_state=data['reconciliation_state'],
                user=request.user, replacement_unit_price=data.get('replacement_unit_price'),
                reference=data.get('reference', ''),
            )
        except RentalException as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
