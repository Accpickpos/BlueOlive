        ...
        with transaction.atomic():
            ...
            if shop_ids:
                valid_shops = Shop.objects.filter(
                    id__in=shop_ids, tenant=tenant, is_active=True
                )
...
def list_tenant_users(request):
    ...
    tenant_id = request.query_params.get("tenant_id") or (
        request.data.get("tenant_id") if request.method == "POST" else None
    )
...
def reset_user_password(request):
    ...
    return Response({"message": f'Password for user "{user.username}" has been reset'})
...
def assign_user_shops(request):
    ...
    if shop_ids:
        valid_shops = Shop.objects.filter(
            id__in=shop_ids, tenant=user.tenant, is_active=True
        )
        user.shops.set(valid_shops)
    else:
        user.shops.clear()
