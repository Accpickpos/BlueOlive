...
class TransactionService(BaseService):
    ...
    def validate_transaction_date(self, date) -> bool:
        ...
        if date > timezone.now().date():
            self.add_error(
                "Transaction date cannot be in the future", "transaction_date"
            )
            return False
        return True
...
class BalanceService(TenantService):
    ...
    def calculate_aging(
        self, account_id: int, model_class, aging_days: List[int] = [30, 60, 90, 120]
    ) -> Dict[str, Decimal]:
        """
        ...
        aging["current"] = model_class.objects.filter(
            **filter_dict, transaction_date__gte=today - timedelta(days=aging_days[0])
...
class VATService(TenantService):
    ...
    def calculate_vat(
        self, amount: Decimal, vat_rate: Decimal, inclusive: bool = False
    ) -> Dict[str, Decimal]:
        """
...
class NumberSequenceService(TenantService):
    ...
    def generate_number(
        self, prefix: str, model_class, number_field: str = "transaction_number"
    ) -> str:
        """
...
class ReconciliationService(TransactionService):
    ...
    def reconcile(
        self, statement_items: List[Dict], transactions: List[Any]
    ) -> Dict[str, Any]:
        """
        ...
        for stmt_item in statement_items:
            ...
            for trans in unmatched_system:
                if self._matches(stmt_item, trans):
                    matched.append({"statement_item": stmt_item, "transaction": trans})
    ...
    def _matches(self, stmt_item: Dict, transaction: Any) -> bool:
        ...
        return stmt_item.get("amount") == getattr(
            transaction, "amount", None
        ) and stmt_item.get("date") == getattr(transaction, "transaction_date", None)
