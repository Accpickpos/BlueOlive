...
from apps.settings.legacy_import_utils import clamp_decimal, open_dbf, to_date, to_decimal, to_int, to_str
...
class Command(TenantAwareLegacyImportCommand):
    ...
    def add_legacy_arguments(self, parser):
        parser.add_argument('--source', type=str, default='smast.dbf', help='Which DBF file to import as the stock master')
    ...
    def run(self, **options):
        ...
        tax_codes = {
            t.code: t for t in TaxCode.objects.using(self.db_alias).all()
        }
        creditors = {
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Stock items imported from {path.name}'))
    ...
    def _import_special_deal(self, item, record):
        ...
        SpecialDeal.objects.using(self.db_alias).update_or_create(
            stock_item=item, start_date=start_date, end_date=end_date,
            defaults={
    ...
    def _import_future_pricing(self, item, record):
        ...
        FuturePricing.objects.using(self.db_alias).update_or_create(
            stock_item=item, effective_date=effective_date,
            defaults={
