    ...
    def run(self, **options):
        ...
        for filename in ('bom.dbf', 'bom5.dbf'):
            ...
            for record in open_dbf(path):
                ...
                try:
                    _, created = PackBundleIngredient.objects.using(self.db_alias).update_or_create(
                        pack_bundle=pack_bundle, ingredient_stock=ingredient,
                        defaults={'quantity': to_decimal(record.get('IQTY')) or 1}
                    )
                    self.created += created
                    self.updated += not created
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f'  ⚠ {mast_code}->{icode}: {e}'))
                    self.errors += 1
