    ...
    def run(self, **options):
        ...
        for filename in ("shrink.dbf", "shrin5.dbf"):
            ...
            for record in open_dbf(path):
                ...
                try:
                    _, created = ShrinkWrap.objects.using(
                        self.db_alias
                    ).update_or_create(
                        shrink_pack_code=shrink_item,
                        bulk_pack_code=bulk_item,
                        defaults={
                            "invoice_bulk_value": to_decimal(record.get("SINBULK")) or 1
                        },
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Shrink wraps imported from {filename}")
            )
