    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            defaults = {
                ...
                "warranty_period_months": to_int(
                    record.get("WARRANTPER"), default=None
                ),
                ...
                "is_stolen": to_str(record.get("STOLEN")).upper()
                in ("Y", "T", "TRUE", "1"),
            }

            try:
                _, created = SerializedStockItem.objects.using(
                    self.db_alias
                ).update_or_create(serial_number=serial_number, defaults=defaults)
                self.created += created
                self.updated += not created
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Serial {serial_number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Serial numbers imported from {path.name}")
        )
