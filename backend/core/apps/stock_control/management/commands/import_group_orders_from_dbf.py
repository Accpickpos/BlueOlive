...
class Command(TenantAwareLegacyImportCommand):
    ...
    def add_legacy_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default="MAIN",
            help="Branch code to attach these group orders to",
        )
    ...
    def run(self, **options):
        ...
        for path in paths:
            ...
            for record in open_dbf(path):
                ...
                try:
                    _, created = GroupOrderItem.objects.using(
                        self.db_alias
                    ).update_or_create(
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"  ⚠ {path.name} {stock_code}: {e}")
                    )
                    self.errors += 1

            GroupOrder.objects.using(self.db_alias).filter(pk=group_order.pk).update(
                total_amount=total
            )

        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Group orders imported from {len(paths)} file(s)")
        )
