...
from apps.settings.legacy_import_utils import open_dbf, parse_hhmm, to_decimal, to_int, to_str
...
class Command(TenantAwareLegacyImportCommand):
    help = 'Import day-of-week/time-window department discounts from smast_discounts.dbf'
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            if department is None or day_of_week is None or not start_time or not end_time:
                self.skipped += 1
                continue

            try:
                _, created = DepartmentTimeDiscount.objects.using(self.db_alias).update_or_create(
                    department=department, day_of_week=day_of_week,
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Department discounts imported from {path.name}'))
