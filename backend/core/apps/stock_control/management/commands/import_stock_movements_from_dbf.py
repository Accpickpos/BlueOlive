...
from apps.settings.legacy_import_utils import (
    code_from_filename, open_dbf, parse_hhmm, parse_yyyymmdd, to_decimal, to_int, to_str,
)
    ...
    def run(self, **options):
        ...
        for path in paths:
            ...
            if stock_item is None:
                self.stdout.write(self.style.WARNING(f'  ⚠ {path.name}: no stock item for code {code!r}, skipping file'))
                self.skipped += 1
                continue
            ...
            for record in open_dbf(path):
                ...
                try:
                    if record_reference:
                        ...
                        _, created = StockMovementLedger.objects.using(self.db_alias).update_or_create(
                            stock_item=stock_item, record_reference=record_reference,
                            movement_date=movement_date, defaults=defaults,
                        )
                        self.created += created
                        self.updated += not created
                    else:
                        StockMovementLedger.objects.using(self.db_alias).create(
                            stock_item=stock_item, movement_date=movement_date,
                            record_reference=None, **defaults,
                        )
                        self.created += 1
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Stock movements imported from {len(paths)} stmove*.dbf files'))
