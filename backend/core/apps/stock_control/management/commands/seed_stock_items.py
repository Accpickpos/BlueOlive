...
class Command(BaseSeedCommand):
    ...
    def _create_stock_items(
        self, user, departments, tax_code, creditors, dry_run=False
    ):
        ...
        for i, item_data in enumerate(stock_items_data):
            ...
            if stock_code in existing_codes:
                self.skip(
                    f"Stock Item {stock_code}: {item_data['description']} (already exists)"
                )
                skipped += 1
                continue
            ...
            dept = (
                dept_list[dept_index] if dept_index < len(dept_list) else dept_list[0]
            )
