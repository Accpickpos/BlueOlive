...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--department-id', type=int, default=1, help='Default department ID')
    ...
    def handle(self, *args, **options):
        try:
            ...
            with open(options['file_path'], 'r', encoding='utf-8') as csvfile:
                ...
                try:
                    department = SalesDepartment.objects.get(id=options['department_id'])
                except SalesDepartment.DoesNotExist:
                    raise CommandError(f'Department with ID {options["department_id"]} not found')
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        if not stock_code:
                            self.stdout.write(f'Row {row_num}: Missing stock_code, skipping')
                            errors += 1
                            continue
                        
                        # Perform update_or_create
                        item, created_flag = StockItem.objects.using(db_alias).update_or_create(
                            stock_code=stock_code,
                            defaults={
                                'description': row.get('description', '')[:255],
                                'department': department,
                                'cost_price': Decimal(row.get('cost_price', 0)) if row.get('cost_price') else Decimal(0),
                                'average_cost': Decimal(row.get('average_cost', 0)) if row.get('average_cost') else Decimal(0),
                                'selling_price_1': Decimal(row.get('selling_price_1', 0)) if row.get('selling_price_1') else Decimal(0),
                                'selling_price_2': Decimal(row.get('selling_price_2', 0)) if row.get('selling_price_2') else Decimal(0),
                                'selling_price_3': Decimal(row.get('selling_price_3', 0)) if row.get('selling_price_3') else Decimal(0),
                                'quantity_on_hand': Decimal(row.get('quantity_on_hand', 0)) if row.get('quantity_on_hand') else Decimal(0),
                                'reorder_quantity': Decimal(row.get('reorder_quantity', 0)) if row.get('reorder_quantity') else Decimal(0),
                                'bin_number': row.get('bin_number', ''),
                                'maximum_discount_percent': Decimal(row.get('maximum_discount_percent', 0)) if row.get('maximum_discount_percent') else Decimal(0),
                                'is_active': row.get('is_active', 'True').lower() in ['true', 'yes', '1', 'y'],
                            }
                        )
