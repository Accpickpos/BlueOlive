...
class StockItemMinimalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockItem
        fields = ['stock_code', 'description', 'selling_price_1', 'cost_price', 'quantity_on_hand']
...
class SpecialDealSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'stock_item',
            'special_cost_price',
            'special_selling_price_1', 'special_selling_price_2', 'special_selling_price_3',
            'special_markup_1', 'special_markup_2', 'special_markup_3',
            'start_date', 'end_date',
...
class FuturePricingSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'stock_item',
            'future_cost_price',
            'future_selling_price_1', 'future_selling_price_2', 'future_selling_price_3',
            'future_markup_1', 'future_markup_2', 'future_markup_3',
            'effective_date', 'is_applied',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['created_at', 'updated_at']
...
class ShrinkWrapSerializer(serializers.ModelSerializer):
    shrink_pack_code_detail = StockItemMinimalSerializer(source='shrink_pack_code', read_only=True)
    bulk_pack_code_detail = StockItemMinimalSerializer(source='bulk_pack_code', read_only=True)

    class Meta:
        model = ShrinkWrap
        fields = [
            'id',
            'shrink_pack_code', 'shrink_pack_code_detail',
            'bulk_pack_code', 'bulk_pack_code_detail',
            'invoice_bulk_value',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['created_at', 'updated_at']
...
class PackBundleIngredientSerializer(serializers.ModelSerializer):
    ingredient_stock_detail = StockItemMinimalSerializer(source='ingredient_stock', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'pack_bundle',
            'ingredient_stock', 'ingredient_stock_detail',
            'quantity', 'cost_at_creation',
            'ingredient_value',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['created_at', 'updated_at']
...
class StockItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            # Identity
            'stock_code', 'description', 'is_active',
            # Relations
            'department', 'supplier', 'supplier_code', 'last_supplier', 'tax_code',
            # Pricing
            ...
            'quantity_on_order', 'quantity_counted',
            'available_quantity',
            'reorder_quantity', 'default_selling_quantity',
            # Settings
            'allow_negative_quantities', 'maximum_discount_percent',
            # Sales stats
...
class StockTransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'transaction_type',
            'stock_item', 'stock_item_detail',
            'transaction_date', 'transaction_time', 'transaction_number',
            'quantity_in', 'quantity_out', 'quantity_balance',
            'discount', 'unit_cost', 'unit_price', 'value',
            'department', 'tax_code', 'debtor', 'supplier',
            'station_number', 'comments',
    ...
    def validate(self, data):
        ...
        if qty_in < 0 or qty_out < 0:
            raise serializers.ValidationError("quantity_in and quantity_out must be non-negative.")
...
class StockMovementLedgerSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'stock_item',
            'movement_date', 'movement_time', 'movement_type',
            'transaction_number',
            'details', 'supplier_details', 'record_reference',
            'quantity_in', 'quantity_out', 'balance',
            'brought_forward', 'per_unit_cost',
            'created_at',
        ]
...
class StockTakeItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'stock_take',
            'stock_item', 'stock_item_detail',
            'quantity_on_hand', 'quantity_counted',
            'variance_quantity', 'variance_value',
            'cost_price_at_count',
            'is_counted', 'count_date',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['variance_quantity', 'variance_value', 'created_at', 'updated_at']
...
class StockTakeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockTake
        fields = [
            'id', 'stock_take_date', 'status', 'description',
            'reset_negatives_to_zero', 'set_uncounted_to_zero',
            'is_after_trading', 'trading_start_date',
...
class ContractPricingSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'debtor', 'stock_item', 'department', 'supplier',
            'pricing_method', 'contract_price', 'markup_percent', 'discount_percent',
            'last_selling_price', 'last_updated_date',
            'is_fixed_pricing', 'is_active',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['created_at', 'updated_at']

    def validate(self, data):
        method = data.get('pricing_method', getattr(self.instance, 'pricing_method', None))
        if method == 'ACTUAL' and not data.get('contract_price') and not getattr(self.instance, 'contract_price', None):
            raise serializers.ValidationError(
                {'contract_price': 'Required when pricing_method is ACTUAL.'}
            )
        if method == 'COST_MARKUP' and data.get('markup_percent') is None and getattr(self.instance, 'markup_percent', None) is None:
            raise serializers.ValidationError(
...
class OneTouchLookupKeySerializer(serializers.ModelSerializer):
    ...
    def validate_key_character(self, value):
        if not value.isupper() or not value.isalpha():
            raise serializers.ValidationError("key_character must be a single uppercase letter (A-Z).")
        return value
...
class StockMonthlyStatisticSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'stock_item', 'year', 'month',
            'quantity_sold', 'value_sold', 'profit_value', 'profit_percent',
            'created_at', 'updated_at',
        ]
        read_only_fields = ['created_at', 'updated_at']
...
class BranchStockSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'stock_item', 'stock_item_detail',
            'quantity', 'quantity_allocated', 'available_quantity',
...
class GroupOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GroupOrder
        fields = [
            'id', 'group_order_number', 'order_date', 'status',
            'branch', 'branch_detail',
            'notes', 'total_amount',
            'created_by',
            'items',
...
class GroupOrderListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'group_order_number', 'order_date', 'status',
            'branch', 'branch_detail', 'total_amount',
        ]
...
class BranchTransferSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'transfer_number',
            'from_branch', 'from_branch_detail',
            'to_branch', 'to_branch_detail',
        ...
        read_only_fields = [
            'approved_date', 'dispatched_date', 'received_date',
            'created_at', 'updated_at',
        ]
...
class BranchTransferListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'from_branch', 'from_branch_detail',
            'to_branch', 'to_branch_detail',
            'status', 'transfer_type', 'requested_date',
        ]
...
class BranchTransferInvoiceSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            'id', 'transfer', 'invoice_number', 'invoice_date', 'status',
