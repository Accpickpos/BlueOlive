...
class CalculationService:
    ...
    def calculate_line_totals(
        ...
        quantize_places: Decimal = Decimal("0.01"),
    ) -> Dict[str, Decimal]:
        """
        ...
        try:
            ...
            rate = (
                (Decimal(str(vat_rate)) / Decimal(100))
                if vat_rate is not None
                else CalculationService.VAT_RATE
            )
    ...
    @staticmethod
    def calculate_tender_balance(
        tenders: List[Dict], total: Decimal
    ) -> Tuple[Decimal, bool]:
        """
    ...
    def apply_settlement_discount(
        amount: Decimal, discount_percentage: Decimal
    ) -> Decimal:
        """
    ...
    @staticmethod
    def calculate_laybye_balance(
        total_amount: Decimal, deposit_amount: Decimal, amount_paid: Decimal = None
    ) -> Dict[str, Decimal]:
        """
        ...
        return {
            ...
            "deposit_percentage": (deposit_amount / total_amount * 100).quantize(
                Decimal("0.01")
            ),
            "percent_paid": (amount_paid / total_amount * 100).quantize(
                Decimal("0.01")
            ),
        }
    ...
    def calculate_profit_metrics(
        total_amount: Decimal, total_cost: Decimal, vat_amount: Decimal = None
    ) -> Dict[str, Decimal]:
        """
        ...
        if taxable_amount == 0:
            ...
            profit_margin = (gross_profit / taxable_amount * 100).quantize(
                Decimal("0.01")
            )
            if total_cost == 0:
                markup_percentage = Decimal(0)
            else:
                markup_percentage = (gross_profit / total_cost * 100).quantize(
                    Decimal("0.01")
                )
        ...
        return {
            ...
            "cost_percentage": (
                (total_cost / taxable_amount * 100).quantize(Decimal("0.01"))
                if taxable_amount > 0
                else Decimal(0)
            ),
        }
    ...
    @staticmethod
    def log_calculation(
        transaction_number: str, operation: str, details: Dict, error: str = None
    ):
        """
