...
TYPE_MAP = {'RE': 'OT', 'RS': 'IS', 'RR': 'RC', 'IS': 'IS', 'RC': 'RC', 'AD': 'AD', 'RT': 'RT'}
...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_master(self):
        ...
        for record in open_dbf(path):
            ...
            status = translate_code(
                self, record.get('STATUS'), STATUS_MAP, 'C', context=f'repmast.dbf STATUS (repair {repair_number})'
            )
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ Repair {repair_number}: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS('  ✓ Repair master imported from repmast.dbf'))
    ...
    def _import_lines(self):
        ...
        repairs = {r.repair_number: r for r in Repair.objects.using(self.db_alias).all()}
        ...
        for record in open_dbf(path):
            ...
            transaction_type = translate_code(
                self, record.get('TYPE'), TYPE_MAP, 'OT', context=f'reptran.dbf TYPE (repair {repair_number})'
            )
            ...
            try:
                ...
                line, created = RepairLine.objects.using(self.db_alias).update_or_create(
                    repair=repair, transaction_type=transaction_type,
                    transaction_date=transaction_date, amount=to_decimal(record.get('AMOUNT')),
                    defaults={
                        'supplier_number': to_int(record.get('SUPNO'), default=None),
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ Repair {repair_number} line: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS('  ✓ Repair lines imported from reptran.dbf'))
