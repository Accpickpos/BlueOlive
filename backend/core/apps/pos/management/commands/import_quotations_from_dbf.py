...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_master(self):
        ...
        for record in open_dbf(path):
            ...
            status = translate_code(
                ...
                STATUS_MAP,
                "ACTIVE",
                context=f"qmast.dbf STATUS (quote {quotation_number})",
            )
            ...
            try:
                _, created = Quotation.objects.using(self.db_alias).update_or_create(
                    ...
                    defaults={
                        ...
                        "debtor_account": debtors.get(
                            to_int(record.get("DNO"), default=None)
                        ),
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Quote {quotation_number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Quotation master imported from qmast.dbf")
        )
    ...
    def _import_lines(self):
        ...
        quotations = {
            q.quotation_number: q for q in Quotation.objects.using(self.db_alias).all()
        }
        stock_items = {
        ...
        for record in open_dbf(path):
            ...
            description = (
                stock_item.description if stock_item else (stock_code or "Unknown item")
            )

            line_counters[quotation_number] = line_counters.get(quotation_number, 0) + 1

            try:
                _, created = QuotationLine.objects.using(
                    self.db_alias
                ).update_or_create(
                    quotation=quotation,
                    line_number=line_counters[quotation_number],
                    defaults={
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Quote {quotation_number} line: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Quotation lines imported from qtran.dbf")
        )
