...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _first_sale_amounts(self):
        ...
        for record in open_dbf(path):
            ...
            if to_str(record.get("TYPE"), 2) == "SP" and lbno not in deposits:
                deposits[lbno] = to_decimal(record.get("QTY")) * to_decimal(
                    record.get("SPRICE")
                )
        return deposits
    ...
    def _import_master(self, deposits):
        ...
        for record in open_dbf(path):
            ...
            status = translate_code(
                ...
                STATUS_MAP,
                "ACTIVE",
                context=f"lbmast.dbf STATUS (laybye {laybye_number})",
            )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Laybye {laybye_number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Laybye master imported from lbmast.dbf")
        )
    ...
    def _import_lines(self):
        ...
        laybyes = {
            lb.laybye_number: lb for lb in Laybye.objects.using(self.db_alias).all()
        }
        ...
        for record in open_dbf(path):
            ...
            transaction_type = translate_code(
                ...
                TYPE_MAP,
                "OT",
                context=f"lbtran.dbf TYPE (laybye {laybye_number})",
            )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Laybye {laybye_number} line: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS("  ✓ Laybye lines imported from lbtran.dbf")
        )
