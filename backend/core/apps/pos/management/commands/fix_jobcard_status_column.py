    ...
    def handle(self, *args, **options):
        ...
        for schema in schemas:
            ...
            try:
                ...
                if not table_exists:
                    self.stdout.write(
                        f"  pos_jobcard table does not exist in schema {schema}"
                    )
                    continue
                ...
                if not result:
                    self.stdout.write(
                        self.style.ERROR(
                            f"  status column not found in schema {schema}"
                        )
                    )
                    continue
                ...
                if max_length and max_length < 25:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  Column is varchar({max_length}), needs to be varchar(25)"
                        )
                    )
                    ...
                    try:
                        # Attempt the ALTER
                        cursor.execute(
                            f'ALTER TABLE "{schema}".pos_jobcard ALTER COLUMN status TYPE varchar(25)'
                        )
                        self.stdout.write(
                            self.style.SUCCESS(f"  ✓ Fixed schema {schema}")
                        )
                        fixed_count += 1
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error processing schema {schema}: {e}")
                )
