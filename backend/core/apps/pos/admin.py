...
class CashSaleLineInline(admin.TabularInline):
    ...
    extra = 1
    fields = [
        'line_number', 'stock_code', 'description', 'quantity',
        'unit_price', 'discount_percentage', 'tax_code',
        'line_total', 'vat_amount', 'line_profit'
    ]
...
class CashSaleAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'sale_number', 'sale_date', 'sale_time', 'customer_name',
        'cashier', 'station_number', 'total_amount_display',
        'gross_profit_display', 'is_posted', 'is_cancelled'
    ]
    
    list_filter = [
        'is_posted', 'is_cancelled', 'sale_date',
        'cashier', 'station_number', 'sales_area'
    ]
    
    search_fields = [
        'sale_number', 'customer_name', 'telephone',
        'order_number', 'job_card_number'
    ]
    
    readonly_fields = [
        'subtotal', 'discount_amount', 'vat_amount',
        'total_amount', 'total_cost', 'gross_profit',
        'change_given', 'created_at', 'updated_at'
    ]
    ...
    fieldsets = (
        ('Sale Details', {
            'fields': (
                'sale_number', 'sale_date', 'sale_time',
                'is_posted', 'is_cancelled'
            )
        }),
        ('Customer Information', {
            'fields': (
                'customer_name', 'delivery_address', 'telephone',
                'vat_number', 'order_number', 'job_card_number'
            )
        }),
        ('POS Details', {
            'fields': (
        ...
        ('Payment', {
            'fields': ('cash_tendered', 'change_given')
        }),
        ('Timestamps', {
...
class LaybyeAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'laybye_number', 'customer_name', 'telephone',
        'laybye_date', 'expiry_date', 'total_amount_display',
        'balance_due_display', 'status'
    ]
    ...
    readonly_fields = ['amount_paid', 'balance_due', 'refund_amount', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ('Laybye Details', {
            'fields': (
                'laybye_number', 'laybye_date', 'expiry_date', 'status'
            )
        }),
        ('Customer Information', {
            'fields': (
                'customer_name', 'address', 'telephone', 'comments'
            )
        }),
        ('Amounts', {
            'fields': (
                'total_amount', 'deposit_amount', 'amount_paid', 'balance_due'
            )
        }),
        ('Cancellation', {
            'fields': (
                'retention_percentage', 'refund_amount'
            )
        }),
        ('Other', {
            'fields': ('sales_area',)
    ...
    def balance_due_display(self, obj):
        ...
        if obj.balance_due > 0:
            return format_html('<span style="color: red;">R {:,.2f}</span>', obj.balance_due)
...
class QuotationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'quotation_number', 'quotation_date', 'expiry_date',
        'customer_name', 'total_amount_display', 'status'
    ]
    ...
    readonly_fields = ['subtotal', 'vat_amount', 'total_amount', 'gross_profit', 'created_at', 'updated_at']
...
class PayoutAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'payout_date', 'payee', 'amount_display',
        'cashier', 'station_number'
    ]
...
class RepairAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'repair_number', 'customer_name', 'date_required',
        'quoted_value_display', 'repair_cost_display', 'status'
    ]
    ...
    fieldsets = (
        ('Repair Details', {
            'fields': (
                'repair_number', 'status', 'date_required',
                'quoted_value', 'repair_details'
            )
        }),
        ('Customer Information', {
            'fields': (
                'customer_name', 'address', 'telephone',
                'contact_person', 'order_number'
            )
        }),
        ('Supplier Details (When Issued)', {
            'fields': (
                'supplier_account', 'date_sent', 'transport_mode',
                'issue_comments', 'company_contact', 'supplier_contact'
            )
        }),
        ('Repair Costs (When Received)', {
            'fields': (
                'date_repaired', 'supplier_invoice_number',
                'repair_cost', 'supplier_comments'
            )
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
...
class JobCardAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'job_number', 'job_date', 'customer_name',
        'registration_number', 'total_amount_display',
        'gross_profit_display', 'status'
    ]
    
    list_filter = ['status', 'job_date', 'sales_area']
    
    search_fields = [
        'job_number', 'customer_name', 'registration_number',
...
class CashControlAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'cash_sales_total_display', 'cash_takings_display',
        'is_cleared'
    ]
    ...
    fieldsets = (
        ('Control Details', {
            'fields': (
                'control_date', 'cashier', 'station_number', 'is_cleared'
            )
        }),
        ('Cash Sales', {
            'fields': (
                'cash_sales_count', 'cash_sales_total',
                'cash_refunds_count', 'cash_refunds_total'
            )
        }),
        ('Invoices and Credits', {
            'fields': (
                'invoices_count', 'invoices_total',
                'credits_count', 'credits_total'
            )
        }),
        ('Receipts', {
            'fields': (
        ...
        ('Cash Returns', {
            'fields': (
                'cash_returns_count', 'cash_returns_total'
            )
        }),
        ('Cash a Cheque', {
            'fields': (
                'cashed_cheques_count', 'cashed_cheques_total',
                'cashed_cheques_commission'
            )
        }),
        ('Laybyes', {
            'fields': (
                'new_laybyes_count', 'new_laybyes_total',
                'cancelled_laybyes_count', 'cancelled_laybyes_total',
                'laybye_receipts_count', 'laybye_receipts_total',
                'laybye_refunds_count', 'laybye_refunds_total',
                'completed_laybyes_count', 'completed_laybyes_total'
            ),
            'classes': ('collapse',)
        }),
        ('Payouts', {
            'fields': ('payouts_count', 'payouts_total')
        }),
        ('Takings by Type', {
            'fields': (
                'cash_takings', 'cheque_takings',
                'voucher_takings', 'speedpoint_takings'
            )
        }),
        ('Refunds by Type', {
            'fields': (
                'cash_refunds', 'cheque_refunds', 'voucher_refunds'
            ),
            'classes': ('collapse',)
        }),
        ('Other', {
            'fields': (
                'rounding_total', 'abandoned_count', 'abandoned_total'
            ),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
...
class ReceiptOnAccountAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'receipt_number', 'receipt_date', 'debtor_account', 'debtor_name',
        'amount_display', 'settlement_discount_display', 'total_amount_display',
        'tender_type', 'is_posted'
    ]
    
    list_filter = ['receipt_date', 'tender_type', 'is_posted', 'cashier', 'station_number']
...
class CreditNoteAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'credit_number', 'credit_date', 'customer_name',
        'total_amount_display', 'refund_type', 'is_posted'
    ]
    
    list_filter = ['credit_date', 'refund_type', 'is_posted', 'cashier', 'station_number']
...
class CashReturnAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'customer_name', 'total_amount_display', 'is_posted'
    ]
...
class CashAChequeAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'transaction_number', 'transaction_date', 'drawer_name',
        'cheque_number', 'bank_name', 'cheque_amount_display',
        'commission_display', 'cash_paid_display', 'is_processed'
    ]
...
class TransactionQueryAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'query_number', 'query_date', 'transaction_type',
        'transaction_number', 'customer_name', 'query_status',
    ...
    fieldsets = (
        ('Query Details', {
            'fields': (
                'query_number', 'query_date', 'transaction_type',
                'transaction_number', 'query_status'
            )
        }),
        ('Customer Information', {
            'fields': (
