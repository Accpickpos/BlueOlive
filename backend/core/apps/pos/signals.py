...
from .models import (
    CashSale, CashSaleLine, Laybye, LaybyeLine, 
    Quotation, QuotationLine, JobCard, JobCardLine,
    CreditNote, CreditNoteLine, CashReturn, CashReturnLine
)
    ...
    if created or kwargs.get('update_fields'):
        try:
            ...
            line_data = [
                {
                    ...
                    'line_cost': (line.quantity * line.cost_price) if line.cost_price else Decimal(0),
                    'discount_amount': (line.quantity * line.unit_price * 
                                      (line.discount_percentage / 100)) if line.discount_percentage else Decimal(0)
                }
                for line in lines
            ]
            ...
            if line_data:
                ...
                cash_sale.save(update_fields=[
                    'subtotal', 'vat_amount', 'total_amount', 'discount_amount',
                    'total_cost', 'gross_profit', 'change_given'
                ])
...
def update_laybye_header_after_line(sender, instance, created, **kwargs):
    ...
    if created or kwargs.get('update_fields'):
        try:
            ...
            if lines.exists():
                total_amount = sum(
                    (line.line_total or Decimal(0)) for line in lines
                )
    ...
    if created or kwargs.get('update_fields'):
        try:
            ...
            if lines.exists():
                ...
                quotation.save(update_fields=['subtotal', 'vat_amount', 'total_amount', 'gross_profit'])
...
def update_creditnote_header_after_line(sender, instance, created, **kwargs):
    ...
    if created or kwargs.get('update_fields'):
        try:
            ...
            if lines.exists():
                subtotal = sum((line.line_total or Decimal(0)) - (line.vat_amount or Decimal(0)) for line in lines)
                ...
                credit_note.save(update_fields=['subtotal', 'vat_amount', 'total_amount'])
    ...
    if created or kwargs.get('update_fields'):
        try:
            ...
            if lines.exists():
                ...
                cash_return.save(update_fields=['subtotal', 'vat_amount', 'total_amount'])
