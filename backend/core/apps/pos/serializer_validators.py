...
from .exceptions import DuplicateTransaction, POSValidationException
    ...
    def validate_lines_list(lines_data):
        ...
        for idx, line in enumerate(lines_data, start=1):
            try:
                ...
                raise serializers.ValidationError({"lines": {idx: e.detail}})
    ...
    def validate_cheque_details(tender_type, cheque_data):
        ...
        for field in required_fields:
            if not cheque_data.get(field):
                raise serializers.ValidationError(
                    {field: "This field is required for cheque tenders."}
                )
