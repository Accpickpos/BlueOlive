...
class PurchaseOrderLineSerializer(serializers.ModelSerializer):
    ...
    stock_description = serializers.CharField(
        source="stock_item.description", read_only=True
    )
    ...
    current_cost = serializers.DecimalField(
        source="stock_item.cost_price", read_only=True, max_digits=10, decimal_places=2
    )
...
class PurchaseOrderDetailSerializer(serializers.ModelSerializer):
    ...
    supplier_name = serializers.CharField(source="supplier.name", read_only=True)
    supplier_account = serializers.CharField(
        source="supplier.our_account_number", read_only=True
    )
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    pricing_method_display = serializers.CharField(
        source="get_pricing_method_display", read_only=True
    )
...
class PurchaseOrderCreateSerializer(serializers.Serializer):
    ...
    pricing_method = serializers.ChoiceField(
        choices=PurchaseOrder.PRICING_METHOD_CHOICES, default="COST"
    )
    ...
    line_items = serializers.ListField(
        child=PurchaseOrderCreateLineSerializer(), required=False, allow_empty=True
    )

    def validate(self, data):
        """Validate that we have either extraction options or manual line items"""
        has_extraction = data.get("extract_all_items") or data.get(
            "extract_below_reorder"
        )
...
class PurchaseOrderReceiptLineSerializer(serializers.ModelSerializer):
    ...
    stock_code = serializers.CharField(
        source="purchase_order_line.stock_item.stock_code", read_only=True
    )
    stock_description = serializers.CharField(
        source="purchase_order_line.stock_item.description", read_only=True
    )
...
class StockReceiptLineSerializer(serializers.Serializer):
    ...
    actual_unit_cost = serializers.DecimalField(
        max_digits=10, decimal_places=2, required=False, allow_null=True
    )
...
class StockReceiptSerializer(serializers.Serializer):
    ...
    create_back_order = serializers.BooleanField(
        default=False, help_text="Create back order for short deliveries"
    )
...
class BackOrderSerializer(serializers.ModelSerializer):
    ...
    original_order_number = serializers.IntegerField(
        source="original_order.order_number", read_only=True
    )
    back_order_number = serializers.IntegerField(
        source="back_order.order_number", read_only=True
    )
    supplier_name = serializers.CharField(
        source="back_order.supplier.name", read_only=True
    )
...
class StockOnOrderSerializer(serializers.Serializer):
    ...
    orders = serializers.ListField(
        child=serializers.DictField(), help_text="List of orders for this item"
    )
