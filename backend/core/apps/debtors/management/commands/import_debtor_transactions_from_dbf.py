...
DTYPE_MAP = {
    'I': 'IN', 'IN': 'IN', 'CN': 'CN', 'C': 'CN', 'CS': 'CS', 'CR': 'CR',
    'P': 'PM', 'PM': 'PM', 'RCP': 'RCP', 'R': 'RCP', 'INT': 'INT',
    'JD': 'JD', 'JC': 'JC', 'RF': 'RF',
}
        ...
        for filename in ('dtran.dbf', 'dtran3.dbf'):
            ...
            for record in open_dbf(path):
                ...
                transaction_type = translate_code(
                    self, record.get('DTYPE'), DTYPE_MAP, 'IN', context=f'{filename} DTYPE (debtor {dno})'
                )
                subtotal = to_decimal(record.get('DTSUB'))
                vat_amount = to_decimal(record.get('DTGST'))
                total_amount = to_decimal(record.get('DTTOT')) or (subtotal + vat_amount)

                if transaction_type in CREDIT_TYPES:
                    subtotal, vat_amount, total_amount = -abs(subtotal), -abs(vat_amount), -abs(total_amount)
                else:
                    subtotal, vat_amount, total_amount = abs(subtotal), abs(vat_amount), abs(total_amount)
                ...
                try:
                    _, created = DebtorTransaction.objects.using(self.db_alias).update_or_create(
                        debtor=debtor, transaction_number=transaction_number, transaction_date=transaction_date,
                        defaults={
            ...
            self.stdout.write(self.style.SUCCESS(f'  ✓ Debtor transactions imported from {filename}'))
