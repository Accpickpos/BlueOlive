...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_master(self):
        ...
        for record in open_dbf(path):
            ...
            status = translate_code(self, record.get('STATUS'), STATUS_MAP, 'A', context=f'jmast.dbf STATUS (job {job_number})')
            ...
            defaults = {
                ...
                'amount_charged': min(to_decimal(record.get('CHARGEAMNT')), to_decimal(record.get('TOTAL')) or Decimal('999999999')),
        ...
        self.stdout.write(self.style.SUCCESS('  ✓ Job costing master imported from jmast.dbf'))
    ...
    def _import_lines(self):
        ...
        job_costings = {j.job_number: j for j in JobCosting.objects.using(self.db_alias).all()}
        ...
        self.stdout.write(self.style.SUCCESS('  ✓ Job costing lines imported from jtran.dbf'))
