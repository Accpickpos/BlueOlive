...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            type_code = translate_code(
                self,
                record.get("TYPE"),
                TYPE_MAP,
                "AD",
                context=f"debtoaud.dbf TYPE (debtor {dno})",
            )
            this_type = translate_code(
                self,
                record.get("THISTYPE"),
                TYPE_MAP,
                type_code,
                context=f"debtoaud.dbf THISTYPE (debtor {dno})",
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Debtor audit imported from {path.name}")
        )
