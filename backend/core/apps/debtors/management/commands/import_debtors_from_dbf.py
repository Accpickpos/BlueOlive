...
from apps.settings.legacy_import_utils import clean_email, open_dbf, to_decimal, to_int, to_str, translate_code
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            defaults = {
                ...
                'discount_printable': 'Y' if to_str(record.get('DISCPRN')).upper() == 'Y' else 'N',
                'dclimit': dclimit,
                'blockflag': translate_code(
                    self, record.get('BLOCKFLAG'), BLOCKFLAG_MAP, '1', context=f'dmast.dbf BLOCKFLAG (debtor {dno})'
                ) if to_str(record.get('BLOCKFLAG')) else '0',
                'dintflag': 'Y' if to_str(record.get('DINTFLAG')).upper() == 'Y' else 'N',
            ...
            try:
                _, created = Debtor.objects.using(self.db_alias).update_or_create(dno=dno, defaults=defaults)
