...
START_PERIOD_CHOICES = {2: '30 days', 3: '60 days', 4: '90 days', 5: '120 days', 6: '150 days', 7: '180 days'}
...
class Command(TenantAwareLegacyImportCommand):
    ...
    def add_legacy_arguments(self, parser):
        ...
        parser.add_argument(
            '--start-period', type=int, default=2, choices=sorted(START_PERIOD_CHOICES),
            help='Charge interest on balances at or beyond this age: ' + ', '.join(
                f'{k}={v}' for k, v in START_PERIOD_CHOICES.items()
            ) + ' (default: 2 = 30 days)'
        )
        parser.add_argument(
            '--charge-credit-balances', action='store_true',
