...
class DebtorFilter(django_filters.FilterSet):
    ...
    account_number = django_filters.CharFilter(
        field_name="dno", lookup_expr="icontains"
    )
    name = django_filters.CharFilter(lookup_expr="icontains")
    search_name = django_filters.CharFilter(
        field_name="dsname", lookup_expr="icontains"
    )

    # Balance filters
    min_balance = django_filters.NumberFilter(field_name="dcrnt", lookup_expr="gte")
    max_balance = django_filters.NumberFilter(field_name="dcrnt", lookup_expr="lte")

    # Credit limit filters
    over_credit_limit = django_filters.BooleanFilter(method="filter_over_credit_limit")
    ...
    cutoff_date = django_filters.DateFilter(method="filter_cutoff_date")
...
class DebtorTransactionFilter(django_filters.FilterSet):
    ...
    # Source filtering
    source_type = django_filters.ChoiceFilter(choices=DebtorTransaction.SOURCE_CHOICES)
    ...
    year = django_filters.NumberFilter(
        field_name="transaction_date", lookup_expr="year", help_text="Filter by year"
    )
    ...
    def filter_age_bucket(self, queryset, name, value):
        ...
        if value == "current":
            return queryset.filter(transaction_date__gte=as_of - timedelta(days=30))
        elif value == "30_60":
            return queryset.filter(
            ...
            return queryset.filter(transaction_date__lt=as_of - timedelta(days=90))
        return queryset
