...
from .serializers import (
    ...
    CreditorCreditNoteLineItemSerializer,
    CreditorCreditNoteSerializer,
    CreditorCreditNoteWriteSerializer,
    CreditorInvoiceLineItemSerializer,
    CreditorInvoiceSerializer,
    CreditorInvoiceWriteSerializer,
    CreditorJournalSerializer,
    CreditorListSerializer,
    CreditorOpenItemSerializer,
...
class CreditorInvoiceLineItemViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return CreditorInvoiceLineItem.objects.filter(
...
class CreditorCreditNoteViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if cn.is_posted:
            return Response(
                {"detail": "Cannot delete a posted credit note."}, status=400
            )
        return super().destroy(request, *args, **kwargs)
...
class CreditorCreditNoteLineItemViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return CreditorCreditNoteLineItem.objects.filter(
            credit_note_id=self.kwargs["cn_pk"]
        )
...
class CreditorPaymentViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def allocate(self, request, pk=None):
        ...
        for item_data in allocations:
            serializer = OpenItemAllocationSerializer(
                data={**item_data, "payment": payment.pk}
            )
...
class SupplierPaymentOrderViewSet(ShopFilterMixin, viewsets.ModelViewSet):
    ...
    def process(self, request, pk=None):
        ...
        SupplierPaymentOrder.objects.filter(pk=order.pk).update(
            is_processed=True, processed_date=timezone.now().date()
        )
