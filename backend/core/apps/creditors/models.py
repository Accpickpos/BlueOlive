...
class Creditor(TimeStampedModel, ActiveModel):
    ...
    supplier_number = models.CharField(
        max_length=4, unique=True, help_text="SUPNO — N(4) in DBF"
    )
    # DBF: SUPNAME C(30)
    ...
    contact_person = models.CharField(
        max_length=20, blank=True, help_text="SUPCONT — C(20)"
    )
    ...
    physical_address_line1 = models.CharField(
        max_length=25, blank=True, help_text="SUPADD1 — C(25)"
    )
    physical_address_line2 = models.CharField(
        max_length=25, blank=True, help_text="SUPADD2 — C(25)"
    )
    physical_address_line3 = models.CharField(
        max_length=25, blank=True, help_text="SUPADD3 — C(25)"
    )

    # --- Postal address: 3 free-form lines matching SUPPADD1/2/3 exactly ---
    postal_address_line1 = models.CharField(
        max_length=20, blank=True, help_text="SUPPADD1 — C(20)"
    )
    postal_address_line2 = models.CharField(
        max_length=20, blank=True, help_text="SUPPADD2 — C(20)"
    )
    postal_address_line3 = models.CharField(
        max_length=20, blank=True, help_text="SUPPADD3 — C(20)"
    )

    # --- Account settings ---
    our_account_number = models.CharField(
        max_length=15, blank=True, help_text="SUPOURACC — C(15)"
    )
    ...
    branch_code = models.CharField(
        max_length=10, blank=True, help_text="BANKCODE — C(10)"
    )
    account_number = models.CharField(
        max_length=15, blank=True, help_text="BANKACC — C(15)"
    )
    ...
    balance_current = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUPCRNT"
    )
    balance_30_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUP30"
    )
    balance_60_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUP60"
    )
    balance_90_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUP90"
    )
    balance_120_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUP120"
    )
    balance_150_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUP150"
    )
    ...
    last_paid_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False, help_text="SUPPMT"
    )
    last_paid_date = models.DateField(
        null=True, blank=True, editable=False, help_text="SUPPMTDATE"
    )
    purchases_mtd = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    purchases_ytd = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
...
class CreditorTransaction(PostingStatusMixin, TimeStampedModel):
    ...
    creditor = models.ForeignKey(
        Creditor, on_delete=models.PROTECT, related_name="%(class)s_set"
    )
    ...
    transaction_reference = models.CharField(
        max_length=20, blank=True, help_text="STREF C(20)"
    )
    additional_reference = models.CharField(
        max_length=200, blank=True, help_text="Django-only, no DBF source"
    )

    # --- Session-captured on save (not entered by user) ---
    grn_number = models.PositiveIntegerField(
        null=True,
        blank=True,
        editable=False,
...
class AgedBalanceMixin(models.Model):
    age_current = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_30 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_60 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_90 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_120 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_150 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
    age_180 = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, editable=False
    )
...
class GoodsReceivedNote(AgedBalanceMixin, CreditorTransaction):
    ...
    inclusive_exclusive = models.CharField(
        max_length=3, choices=INCL_EXCL, default="EXC"
    )
...
class GRNLineItem(TimeStampedModel):
    ...
    stock_item = models.ForeignKey(
        "stock_control.StockItem", on_delete=models.PROTECT, related_name="grn_lines"
    )
...
class CreditorInvoice(AgedBalanceMixin, CreditorTransaction):
    ...
    supplier_invoice_number = models.CharField(
        max_length=50, help_text="TRANO in supexpt"
    )
    inclusive_exclusive = models.CharField(
        max_length=3, choices=INCL_EXCL, default="INC"
    )
    station_no_area = models.CharField(
        max_length=2, blank=True, help_text="SOURCE in supexpt"
    )

    # FIX #10: tax_indicator was missing from original
    tax_indicator = models.PositiveSmallIntegerField(
        default=1, help_text="TAXIND in supexpt. 1=standard VAT, 0=exempt/zero-rated."
    )
    ...
    def save(self, *args, **kwargs):
        self.transaction_type = (
            "IN"  # matches TRANSACTION_TYPE_CHOICES; field is max_length=2
        )
        if not self.transaction_number:
...
class CreditorInvoiceLineItem(TimeStampedModel):
    invoice = models.ForeignKey(
        CreditorInvoice, on_delete=models.CASCADE, related_name="line_items"
    )
    line_number = models.PositiveSmallIntegerField(default=1)
...
class CreditorCreditNoteLineItem(TimeStampedModel):
    credit_note = models.ForeignKey(
        CreditorCreditNote, on_delete=models.CASCADE, related_name="line_items"
    )
    line_number = models.PositiveSmallIntegerField(default=1)
...
class CreditorJournal(AgedBalanceMixin, CreditorTransaction):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.transaction_number:
            ...
            last = (
                CreditorJournal.objects.filter(journal_type=self.journal_type)
                .order_by("-id")
                .first()
            )
            try:
...
class SupplierLedgerEntry(TimeStampedModel):
    ...
    transaction_number = models.CharField(
        max_length=10, db_index=True, help_text="STRANO C(10)"
    )
    ...
    transaction_type = models.CharField(
        max_length=2, choices=STYPE_CHOICES, help_text="STYPE C(2)"
    )

    subtotal = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="STSUB"
    )
    vat_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="STGST"
    )
    total_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="STTOT"
    )
...
class CreditorOpenItem(models.Model):
    ...
    creditor = models.ForeignKey(
        Creditor, on_delete=models.CASCADE, related_name="open_items"
    )
    ...
    age_period = models.PositiveSmallIntegerField(
        default=0, editable=False, help_text="Ageing period bucket — set by ageing run"
    )
...
class OpenItemAllocation(models.Model):
    ...
    payment = models.ForeignKey(
        CreditorPayment, on_delete=models.CASCADE, related_name="allocations"
    )
    open_item = models.ForeignKey(
        CreditorOpenItem, on_delete=models.CASCADE, related_name="allocations"
    )
    ...
    def clean(self):
        ...
        if (
            existing + self.amount_paid + self.settlement_discount
            > self.open_item.balance_due
        ):
            raise ValidationError("Total allocation exceeds balance due")

    def __str__(self):
        return (
            f"{self.payment.transaction_number} → {self.open_item.transaction_number}"
        )
...
class OpenItemAudit(TimeStampedModel):
    ...
    creditor = models.ForeignKey(
        Creditor, on_delete=models.CASCADE, related_name="open_item_audits"
    )
    transaction_number = models.CharField(
        max_length=10, editable=False, help_text="TRANO C(10)"
    )
    transaction_type = models.CharField(
        max_length=2, editable=False, help_text="TYPE C(2)"
    )

    this_transaction_type = models.CharField(
        max_length=2, editable=False, help_text="THISTYPE C(2)"
    )
    # FIX #9: IntegerField — THISTRAN is N(6) integer in DBF, not decimal
    this_transaction_number = models.IntegerField(
        editable=False, help_text="THISTRAN — N(6) in DBF"
    )

    transaction_date = models.DateField(editable=False, help_text="DATE")
    amount = models.DecimalField(
        max_digits=14, decimal_places=2, editable=False, help_text="AMOUNT N(14,2)"
    )
...
class RFC(TimeStampedModel):
    ...
    creditor = models.ForeignKey(
        Creditor, on_delete=models.PROTECT, related_name="rfcs"
    )
    # DBF: RFCNO N(6) — stored as char for display consistency
    rfc_number = models.CharField(
        max_length=6, unique=True, help_text="RFCNO — N(6) in DBF"
    )

    # FIX #5: nullable — no return_date equivalent in supcrmas.dbf; set from DATESENT on import
    return_date = models.DateField(
        null=True, blank=True, help_text="No DBF source — set from DATESENT on import."
    )
    date_sent = models.DateField(null=True, blank=True, help_text="DATESENT")
    date_returned = models.DateField(
        null=True, blank=True, help_text="DATERETN — date credit note received"
    )

    status = models.CharField(
        max_length=2, choices=STATUS_CHOICES, default="PE", help_text="STATUS C(2)"
    )
...
class RFCLineItem(TimeStampedModel):
    ...
    quantity_returned = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="QTYRFC N(10,2)"
    )
    ...
    line_value = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="VAL N(12,2)"
    )
    tax_code = models.ForeignKey(TaxCode, on_delete=models.PROTECT)

    # supcrtrn: TIME is C(5) e.g. "08:30" — stored as char
    rfc_line_date = models.DateField(
        null=True,
        blank=True,
        help_text="DATE — date this line was processed in supcrtrn",
    )
    rfc_line_time = models.CharField(
        max_length=5,
        blank=True,
    ...
    original_transaction_type = models.CharField(
        max_length=2, blank=True, help_text="TYPE C(2)"
    )
    original_transaction_date = models.DateField(
        null=True, blank=True, help_text="PURCHDATE"
    )
    supplier_reference_number = models.CharField(
        max_length=10, blank=True, help_text="SUPREFNO C(10)"
    )
    ...
    # --- System-calculated on save ---
    line_value_exclusive = models.DecimalField(
        max_digits=15, decimal_places=2, editable=False, default=0
    )
    tax_amount = models.DecimalField(
        max_digits=15, decimal_places=2, editable=False, default=0
    )
    line_value_inclusive = models.DecimalField(
        max_digits=15, decimal_places=2, editable=False, default=0
    )
...
class ExpenseCategoryMonthlyBalance(TimeStampedModel):
    ...
    exp_month_1 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_2 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_3 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_4 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_5 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_6 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_7 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_8 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_9 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_10 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_11 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
    ...
    exp_month_12 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
...
class ExpenseCategoryTransaction(TimeStampedModel):
    ...
    transaction_date = models.DateField(help_text="DATE")
    transaction_number = models.CharField(
        max_length=10, db_index=True, help_text="TRANO C(10)"
    )
    ...
    source_type = models.CharField(
        max_length=2, blank=True, help_text="SOURCE C(2) — same codes as suptran STYPE"
    )
    grn_number = models.PositiveIntegerField(
        null=True, blank=True, help_text="GRNNO N(6)"
    )

    # --- System-calculated on save ---
    input_vat_amount = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
        editable=False,
...
class OutstandingBalance(TimeStampedModel):
    ...
    balance_current = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="Current (0-30 days)"
    )
    balance_30_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="30 days overdue"
    )
    balance_60_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="60 days overdue"
    )
    balance_90_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="90 days overdue"
    )
    balance_120_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="120 days overdue"
    )
    balance_150_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="150 days overdue"
    )
    balance_180_days = models.DecimalField(
        max_digits=15, decimal_places=2, default=0, help_text="180+ days overdue"
    )

    # Optional reference info
    supplier_account_number = models.CharField(
        max_length=15, blank=True, help_text="Supplier's account number reference"
    )
    transaction_number = models.CharField(
        max_length=20, blank=True, help_text="Transaction reference number"
    )
    transaction_date = models.DateField(
        null=True, blank=True, help_text="Transaction date if applicable"
    )
    original_amount = models.DecimalField(
        max_digits=15,
        decimal_places=2,
...
class CreditorTransactionLine(TimeStampedModel):
    ...
    tax_code = models.ForeignKey(
        TaxCode, on_delete=models.PROTECT, related_name="transaction_lines"
    )
