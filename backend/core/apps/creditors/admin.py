...
from .models import (
    ...
    RFC, RFCLineItem, CreditorTransactionLine, ExpenseCategoryMonthlyBalance,
    OpenItemAudit
)
...
class CreditorAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'supplier_number', 'name', 'get_balance_color', 'get_aging_summary',
        'credit_terms', 'last_paid_date', 'is_active'
    )
    list_filter = (
        'is_active', 'account_category', BalanceRangeFilter, OverdueFilter,
        'credit_terms', 'sales_area', 'created_at'
    )
    ...
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('supplier_number', 'name', 'contact_person', 'sales_area', 'is_active')
        }),
        (_('Contact Details'), {
            'fields': ('telephone', 'fax', 'email')
        }),
        (_('Address Details'), {
            'fields': (
        ...
        (_('Account Settings'), {
            'fields': (
                ...
                'prompt_payment_discount_percent', 'update_selling_price_on_receipt'
            )
        }),
        (_('Banking Details'), {
        ...
        (_('Opening Balance (BBF)'), {
            ...
            'description': mark_safe('<strong>Set opening balance from previous period</strong>')
        }),
        ...
        (_('Purchase Statistics'), {
            'fields': ('purchases_mtd', 'purchases_ytd'),
            'classes': ('collapse',)
        }),
        (_('Timestamps'), {
    ...
    actions = ['activate_creditors', 'deactivate_creditors', 'recalculate_aged_balances']
    ...
    def get_aging_summary(self, obj):
        ...
        if obj.balance_30_days > 0 or obj.balance_60_days > 0 or obj.balance_90_days > 0:
    ...
    def get_aging_display(self, obj):
        ...
        '''.format(
            obj.balance_current, obj.balance_30_days, obj.balance_60_days,
            obj.balance_90_days, obj.balance_120_days, obj.balance_150_days,
...
class GRNLineItemInline(admin.TabularInline):
    ...
    extra = 1
    fields = (
        'line_number', 'stock_item', 'quantity_received', 'unit_cost',
        'tax_code', 'line_subtotal', 'tax_amount', 'line_total'
    )
...
class GoodsReceivedNoteAdmin(admin.ModelAdmin):
    ...
    list_display = (
        ...
        'supplier_invoice_number', 'get_total_display', 'get_status_badge'
    )
    ...
    readonly_fields = (
        'subtotal', 'total_vat', 'total_quantity', 'transaction_type',
        'transaction_number', 'posted_at', 'posted_by', 'created_at', 'updated_at'
    )
...
class CreditorInvoiceLineItemInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('line_number', 'expense_category', 'amount', 'tax_code', 'tax_amount', 'line_total')
    readonly_fields = ('tax_amount', 'line_total')

    def get_queryset(self, request):
        return super().get_queryset(request).select_related('expense_category', 'tax_code')
...
class CreditorInvoiceAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
        'subtotal', 'total_vat', 'transaction_type',
        'transaction_number', 'posted_at', 'posted_by', 'due_date',
        'created_at', 'updated_at'
    )

    fieldsets = (
        (_('Document Information'), {
            'fields': ('transaction_number', 'transaction_type', 'transaction_date', 'due_date')
        }),
        (_('Supplier Details'), {
            'fields': ('creditor', 'supplier_invoice_number', 'supplier_reference')
        }),
        ...
        (_('Totals'), {
            'fields': ('subtotal', 'total_vat', 'total_amount'),
            'classes': ('wide',)
        }),
        (_('Posting'), {
            'fields': ('is_posted', 'posted_date', 'posted_by', 'posted_at'),
...
class CreditorCreditNoteLineItemInline(admin.TabularInline):
    ...
    extra = 1
    fields = (
        'line_number', 'stock_item', 'quantity_returned', 'unit_cost',
        'tax_code', 'line_subtotal', 'tax_amount', 'line_total'
    )
    readonly_fields = ('line_subtotal', 'tax_amount', 'line_total')
...
class CreditorCreditNoteAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'transaction_number', 'creditor', 'transaction_date',
        'supplier_credit_note_number', 'get_total_display', 'get_status_badge'
    )
    list_filter = ('is_posted', 'transaction_date', 'inclusive_exclusive', 'created_at')
    ...
    fieldsets = (
        ...
        (_('Supplier Details'), {
            'fields': ('creditor', 'supplier_credit_note_number', 'supplier_reference')
        }),
        (_('Credit Note Settings'), {
            'fields': ('inclusive_exclusive',)
        }),
        (_('Totals'), {
            'fields': ('subtotal', 'total_vat', 'total_amount'),
...
class CreditorPaymentAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'transaction_number', 'creditor', 'transaction_date',
        'payment_method', 'get_amount_display', 'get_status_badge'
    )
    list_filter = ('is_posted', 'payment_method', 'transaction_date', 'created_at')
    search_fields = ('transaction_number', 'creditor__name', 'cheque_number', 'reference_number')
    date_hierarchy = 'transaction_date'

    readonly_fields = (
        'settlement_discount_amount', 'settlement_discount_percent',
        'transaction_type', 'transaction_number', 'posted_at',
    ...
    fieldsets = (
        ...
        (_('Payment Details'), {
            'fields': (
                'payment_method', 'amount_paid', 'settlement_discount_percent',
                'settlement_discount_amount', 'total_allocation'
            ),
            'classes': ('wide',)
        }),
        (_('Payment Reference'), {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('creditor', 'posted_by', 'payment_method')
...
class CreditorJournalAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'transaction_number', 'creditor', 'transaction_date',
        'journal_type', 'get_amount_display', 'get_status_badge'
    )
    ...
    readonly_fields = (
        'transaction_type', 'transaction_number', 'total_amount',
        'posted_at', 'posted_by', 'created_at', 'updated_at'
    )

    fieldsets = (
        (_('Journal Information'), {
            'fields': ('transaction_number', 'transaction_type', 'transaction_date', 'journal_type')
        }),
        (_('Creditor Details'), {
            'fields': ('creditor',)
        }),
...
class CreditorOpenItemAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'transaction_number', 'creditor', 'transaction_date',
        'original_amount', 'get_balance_display', 'get_allocation_badge'
    )
    list_filter = ('transaction_date', 'transaction_type', 'is_fully_allocated', 'created_at')
    search_fields = ('transaction_number', 'creditor__name')
    date_hierarchy = 'transaction_date'

    readonly_fields = (
        'transaction_type', 'transaction_number', 'original_amount',
        'balance_due', 'is_fully_allocated', 'created_at', 'updated_at'
    )
    ...
    fieldsets = (
        ...
        (_('Amounts'), {
            'fields': ('original_amount', 'balance_due'),
            'classes': ('wide',)
        }),
        (_('Status'), {
...
class OpenItemAllocationAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'id', 'get_creditor_display', 'open_item', 'payment',
        'get_amount_display', 'allocated_at'
    )
    list_filter = ('allocated_at',)
    search_fields = ('payment__creditor__name', 'open_item__transaction_number', 'payment__transaction_number')
...
class RFCLineItemInline(admin.TabularInline):
    ...
    fields = (
        'line_number', 'stock_item', 'quantity_returned', 'unit_cost',
        'tax_code', 'reason', 'line_value_exclusive', 'tax_amount', 'line_value_inclusive'
    )
...
class RFCAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'rfc_number', 'creditor', 'return_date', 'get_status_badge',
        'get_total_display', 'get_lifecycle_display'
    )
    ...
    fieldsets = (
        (_('RFC Information'), {
            'fields': ('rfc_number', 'return_date', 'status')
        }),
        (_('Creditor Details'), {
            'fields': ('creditor', 'purchase_order_number', 'purchase_order_line_number')
        }),
        (_('Return Details'), {
            'fields': ('reason_for_return', 'remarks'),
        }),
        (_('Lifecycle'), {
            'fields': ('date_sent', 'date_returned', 'credited_date', 'replaced_date', 'replaced_rfc_number'),
            'classes': ('collapse',)
        }),
        (_('Totals'), {
