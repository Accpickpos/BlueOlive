...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = SupplierLedgerEntry.objects.using(self.db_alias).update_or_create(
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Supplier ledger imported from {path.name}'))
