...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        if default_tax_code is None:
            self.stdout.write(
                self.style.ERROR(
                    "  ✗ No TaxCode exists — seed tax codes before importing RFCs"
                )
            )
            return
    ...
    def _import_master(self):
        ...
        for record in open_dbf(path):
            ...
            if creditor is None:
                self.stdout.write(
                    self.style.WARNING(
                        f"  ⚠ RFC {rfc_number}: unknown supplier, skipping"
                    )
                )
                self.skipped += 1
                continue
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  ✓ RFC master imported from {path.name}")
        )
    ...
    def _import_lines(self, default_tax_code):
        ...
        for record in open_dbf(path):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ RFC {rfc_number} line {line_number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS(f"  ✓ RFC lines imported from {path.name}")
        )
