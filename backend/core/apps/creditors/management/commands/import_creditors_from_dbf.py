    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            defaults = {
                ...
                "account_category": ACCTYPE_MAP.get(
                    to_str(record.get("ACCTYPE")), "BBF"
                ),
                "update_selling_price_on_receipt": to_str(
                    record.get("UPDSTKSP")
                ).upper()
                in ("Y", "YES", "T", "TRUE", "1"),
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ⚠ Supplier {supplier_number}: {e}")
                )
                self.errors += 1

        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Creditors imported from {path.name}")
        )
