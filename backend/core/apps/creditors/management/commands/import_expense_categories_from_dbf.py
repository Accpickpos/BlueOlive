...
class Command(TenantAwareLegacyImportCommand):
    ...
    def add_legacy_arguments(self, parser):
        parser.add_argument('--year', type=int, required=True, help='Financial year these balances belong to (supexp.dbf has no year column)')
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = ExpenseCategoryMonthlyBalance.objects.using(self.db_alias).update_or_create(
                    expense_category=category, year=year, defaults=defaults
                )
                self.created += created
                self.updated += not created
            except Exception as e:
                self.stdout.write(self.style.WARNING(f'  ⚠ category {category.number}: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS(f'  ✓ Expense category balances imported from {path.name}'))
