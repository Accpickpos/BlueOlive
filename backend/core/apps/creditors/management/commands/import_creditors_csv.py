...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--credit-terms-id', type=int, default=1, help='Default credit terms ID')
    ...
    def handle(self, *args, **options):
        try:
            ...
            with open(options['file_path'], 'r', encoding='utf-8') as csvfile:
                ...
                try:
                    credit_terms = CreditTerms.objects.get(id=options['credit_terms_id'])
                except CreditTerms.DoesNotExist:
                    raise CommandError(f'Credit terms with ID {options["credit_terms_id"]} not found')
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        if not supplier_number:
                            self.stdout.write(f'Row {row_num}: Missing supplier_number, skipping')
                            errors += 1
                            continue
                        
                        # Perform update_or_create
                        creditor, created_flag = Creditor.objects.using(db_alias).update_or_create(
                            ...
                            defaults={
                                ...
                                'physical_address_line1': row.get('physical_address_line1', '')[:100],
                                'physical_address_line2': row.get('physical_address_line2', '')[:100],
                                'physical_city': row.get('physical_city', '')[:50],
                                'physical_province': row.get('physical_province', '')[:50],
                                'physical_code': row.get('physical_code', '')[:10],
                                'postal_address_line1': row.get('postal_address_line1', '')[:100],
                                'postal_address_line2': row.get('postal_address_line2', '')[:100],
                                'postal_city': row.get('postal_city', '')[:50],
                                'postal_province': row.get('postal_province', '')[:50],
                                'postal_code': row.get('postal_code', '')[:10],
                                'our_account_number': row.get('our_account_number', '')[:50],
