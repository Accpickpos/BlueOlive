...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            ledger_entry = SupplierLedgerEntry.objects.using(self.db_alias).filter(
                creditor=creditor, transaction_number=transaction_number
            ).first()

            try:
                _, created = CreditorOpenItem.objects.using(self.db_alias).update_or_create(
                    creditor=creditor, transaction_number=transaction_number, transaction_type=transaction_type,
                    defaults={
                ...
                self.stdout.write(self.style.WARNING(f'  ⚠ {creditor.supplier_number}/{transaction_number}: {e}'))
                self.errors += 1

        self.stdout.write(self.style.SUCCESS(f'  ✓ Creditor open items imported from {path.name}'))
