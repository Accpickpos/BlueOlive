...
class Command(TenantAwareLegacyImportCommand):
    ...
    def run(self, **options):
        ...
        for record in open_dbf(path):
            ...
            try:
                _, created = ExpenseCategoryTransaction.objects.using(self.db_alias).update_or_create(
                    expense_category=category, creditor=creditor, transaction_number=transaction_number,
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Expense transactions imported from {path.name}'))
