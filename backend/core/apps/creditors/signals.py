...
from .models import (
    ...
    CreditorPayment, CreditorJournal,
...
def creditor_creditnote_lineitem_post_save(sender, instance, created, **kwargs):
    ...
    totals = CreditorCreditNoteLineItem.objects.filter(credit_note=credit_note).aggregate(
...
def open_item_allocation_post_save(sender, instance, created, **kwargs):
    ...
    # for auto-numbering.
    try:
        this_transaction_number = int(instance.payment.transaction_number.split('-')[-1])
    ...
    OpenItemAudit.objects.create(
        ...
        transaction_date=instance.allocated_at.date() if instance.allocated_at else datetime.now().date(),
...
def creditor_journal_post_save(sender, instance, created, **kwargs):
    ...
    signed_amount = instance.journal_amount if is_debit else instance.journal_amount * Decimal('-1')
