    ...
    def validate_number(self, value):
        ...
        if value < 1 or value > 999:
            raise serializers.ValidationError(
                "Department number must be between 1 and 999"
            )

        # Check uniqueness excluding current instance
        instance = self.instance
        if (
            SalesDepartment.objects.filter(number=value)
...
class SalesAreaSerializer(serializers.ModelSerializer):
    ...
    def validate_number(self, value):
        ...
        if value < 1 or value > 99:
            raise serializers.ValidationError(
                "Sales area number must be between 1 and 99"
            )

        instance = self.instance
        if (
            SalesArea.objects.filter(number=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class IncomeCategorySerializer(serializers.ModelSerializer):
    ...
    def validate_number(self, value):
        ...
        if value < 1 or value > 99999999:
            raise serializers.ValidationError(
                "Income category number must be between 1 and 99999999"
            )

        instance = self.instance
        if (
            IncomeCategory.objects.filter(number=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class ExpenseCategorySerializer(serializers.ModelSerializer):
    ...
    def validate_number(self, value):
        ...
        if value < 1 or value > 99999999:
            raise serializers.ValidationError(
                "Expense category number must be between 1 and 99999999"
            )

        instance = self.instance
        if (
            ExpenseCategory.objects.filter(number=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class TaxCodeSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, value):
        ...
        instance = self.instance
        if (
            TaxCode.objects.filter(code=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class CostingCategorySerializer(serializers.ModelSerializer):
    ...
    methods_display = serializers.CharField(
        source="get_methods_display", read_only=True
    )
...
class PaymentMethodSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, value):
        ...
        instance = self.instance
        if (
            PaymentMethod.objects.filter(code=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class CreditTermsSerializer(serializers.ModelSerializer):
    ...
    def validate_days(self, value):
        ...
        instance = self.instance
        if (
            CreditTerms.objects.filter(days=value)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
            raise serializers.ValidationError(
                "Credit terms with these days already exist"
            )
...
class SystemConfigurationSerializer(serializers.ModelSerializer):
    ...
    default_tax_code_details = TaxCodeListSerializer(
        source="default_tax_code", read_only=True
    )
    ...
    def validate_ageing_periods(self, value):
        ...
        for period in value:
            if not isinstance(period, int) or period <= 0:
                raise serializers.ValidationError(
                    "All ageing periods must be positive integers"
                )
...
class DepartmentMonthlyStatsSerializer(serializers.ModelSerializer):
    ...
    department_number = serializers.IntegerField(
        source="department.number", read_only=True
    )
...
class SalesAreaMonthlyStatsSerializer(serializers.ModelSerializer):
    ...
    sales_area_number = serializers.IntegerField(
        source="sales_area.number", read_only=True
    )
    ...
    def create(self, validated_data):
        ...
        if not validated_data.get("tenant"):
            ...
            raise ValidationError(
                {"tenant": "Tenant context required to create API keys"}
            )
