...
class VATMixin(models.Model):
    ...
    value_excl_vat = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="Amount excluding VAT"
    )
    tax_amount = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="VAT/Tax amount"
    )
    ...
    def validate_vat_integrity(self):
        ...
        if abs(self.total_incl_vat - expected_total) > tolerance:
            return (
                False,
                f"VAT mismatch: {self.value_excl_vat} + {self.tax_amount} ≠ {self.total_incl_vat}",
            )
...
class PostingStatusMixin(models.Model):
    ...
    is_posted = models.BooleanField(
        default=False, help_text="Whether this transaction has been posted to GL"
    )
    posted_at = models.DateTimeField(
        null=True, blank=True, editable=False, help_text="Timestamp when posted to GL"
    )
...
class ReconciliationMixin(models.Model):
    ...
    reconciled_at = models.DateTimeField(
        null=True, blank=True, editable=False, help_text="Timestamp when reconciled"
    )
...
class SalesDepartment(TimeStampedModel, ActiveModel):
    ...
    sales_p1 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 1",
    )
    sales_p2 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 2",
    )
    sales_p3 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 3",
    )
    sales_p4 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 4",
    )
    sales_p5 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 5",
    )
    sales_p6 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 6",
    )
    sales_p7 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 7",
    )
    sales_p8 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 8",
    )
    sales_p9 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 9",
    )
    sales_p10 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 10",
    )
    sales_p11 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 11",
    )
    sales_p12 = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=0,
        editable=False,
        help_text="Sales Period 12",
    )
...
class SalesArea(TimeStampedModel, ActiveModel):
    ...
    name = models.CharField(max_length=100, help_text="Salesman name or area name")
...
class ExpenseCategory(TimeStampedModel, ActiveModel):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="Expense category name"
    )
...
class TaxCode(TimeStampedModel, ActiveModel):
    ...
    description = models.CharField(
        max_length=100, help_text="Tax description (e.g., '14% VAT', '0% Zero-rated')"
    )
...
class CostingCategory(TimeStampedModel, ActiveModel):
    ...
    def get_methods_display(self):
        ...
        return (
            f"{self.get_costing_method_display()} | {self.get_pricing_method_display()}"
        )
...
class PaymentMethod(TimeStampedModel, ActiveModel):
    ...
    requires_reference = models.BooleanField(
        default=False, help_text="Requires reference number (e.g., cheque number)"
    )
...
class CreditTerms(TimeStampedModel, ActiveModel):
    ...
    description = models.CharField(
        max_length=100, help_text="Description (e.g., 'COD', '30 Days', '60 Days')"
    )
...
class SystemConfiguration(TimeStampedModel):
    ...
    shop_name = models.CharField(
        max_length=200, blank=True, help_text="Shop/Company name"
    )
    ...
    ageing_periods = models.JSONField(
        default=list, help_text="Ageing periods in days [30, 60, 90, 120, 150, 180]"
    )
    ...
    charge_interest_on_overdue = models.BooleanField(
        default=False, help_text="Charge interest on overdue debtor accounts"
    )
    ...
    date_format = models.CharField(
        max_length=20, default="YYYY-MM-DD", help_text="Date display format"
    )
    currency_symbol = models.CharField(
        max_length=5, default="R", help_text="Currency symbol (e.g., R, $, €)"
    )
...
class DepartmentMonthlyStats(models.Model):
    ...
    department = models.ForeignKey(
        SalesDepartment, on_delete=models.CASCADE, related_name="monthly_stats"
    )
...
class SalesAreaMonthlyStats(models.Model):
    ...
    sales_area = models.ForeignKey(
        SalesArea, on_delete=models.CASCADE, related_name="monthly_stats"
    )
...
class DayEndReport(TimeStampedModel):
    ...
    shop_id = models.IntegerField(
        null=True, blank=True, help_text="Shop filter used for this run, if any"
    )
    ...
    def __str__(self):
        return f"Day End {self.process_date}" + (
            f" (shop {self.shop_id})" if self.shop_id else ""
        )
...
class APIKey(TimeStampedModel):
    ...
    key = models.CharField(
        max_length=40, unique=True, db_index=True, help_text="Secret API key"
    )
    ...
    rate_limit_window = models.PositiveIntegerField(
        default=3600, help_text="Time window in seconds for rate limiting"
    )
    ...
    last_ip = models.GenericIPAddressField(
        null=True, blank=True, help_text="Last IP address that used this key"
    )
