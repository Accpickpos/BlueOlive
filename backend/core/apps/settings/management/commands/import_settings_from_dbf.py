...
class Command(TenantAwareLegacyImportCommand):
    ...
    def _import_departments(self):
        ...
        if not path.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠ {path} not found, skipping departments")
            )
            return
        for record in open_dbf(path):
            ...
            if not number or not name:
                ...
                continue
            name = dedupe_name(
                SalesDepartment.objects.using(self.db_alias),
                name,
                "number",
                number,
                100,
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Departments imported from {path.name}")
        )
    ...
    def _import_sales_areas(self):
        ...
        if not path.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠ {path} not found, skipping sales areas")
            )
            return
        for record in open_dbf(path):
            ...
            if not code or not name:
                ...
                continue
            monthly = {
                f"arsls{i}": to_decimal(record.get(f"ARSLS{i}")) for i in range(1, 13)
            }
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Sales areas imported from {path.name}")
        )
        ...
        for filename in ("cbexp.dbf", "supexp.dbf"):
            ...
            for record in open_dbf(path):
                ...
                if not number or not name:
                    ...
                    continue
                name = dedupe_name(
                    ExpenseCategory.objects.using(self.db_alias),
                    name,
                    "number",
                    number,
                    100,
                )
                _, created = ExpenseCategory.objects.using(
                    self.db_alias
                ).update_or_create(number=number, defaults={"name": name})
                self.created += created
                self.updated += not created
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Expense categories imported from {filename}")
            )
    ...
    def _import_income_categories(self):
        ...
        if not path.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠ {path} not found, skipping income categories")
            )
            return
        for record in open_dbf(path):
            ...
            if not number or not name:
                ...
                continue
            name = dedupe_name(
                IncomeCategory.objects.using(self.db_alias), name, "number", number, 100
            )
            ...
            self.updated += not created
        self.stdout.write(
            self.style.SUCCESS(f"  ✓ Income categories imported from {path.name}")
        )
