...
from apps.settings.seeding_utils import SeedingException, get_shop, get_tenant
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--tenant-id", type=int, help="Tenant ID to seed (defaults to first tenant)"
        )
        parser.add_argument("--tenant-slug", type=str, help="Tenant slug to seed")
        parser.add_argument(
            "--shop-id",
            type=int,
            help="Shop ID to seed (defaults to first active shop)",
        )
        parser.add_argument("--shop-slug", type=str, help="Shop slug to seed")
        parser.add_argument(
            "--dry-run",
            action="store_true",
        ...
        parser.add_argument(
            "--list", action="store_true", help="List all available tenants and shops"
        )
    ...
    def _print_context(self, tenant, shop, dry_run):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"   Shop: {shop.name} (schema: {shop.schema_name})")
        )
    ...
    def _list_tenants(self):
        ...
        if not tenants.exists():
            ...
            self.stdout.write(
                "Create a tenant first: python manage.py create_dev_tenant"
            )
            return
        ...
        for tenant in tenants:
            ...
            if shops.exists():
                ...
                for shop in shops:
                    ...
                    self.stdout.write(
                        f"     {marker} {shop.name} (ID: {shop.id}, schema: {shop.schema_name})"
                    )
            else:
                self.stdout.write("   Shops: ⚠️  None found")
