        ...
        if tenant_id:
            ...
            if not tenant:
                raise SeedingException(
                    "No tenants exist. Create a tenant first using create_dev_tenant command."
                )
            return tenant
...
def get_shop(tenant, shop_id=None, shop_slug=None):
    ...
    try:
        if shop_id:
            ...
            if not shop:
                raise SeedingException(
                    f"No active shops found for tenant '{tenant.name}'. Create a shop first."
                )
            return shop
    except Shop.DoesNotExist:
        if shop_id:
            raise SeedingException(
                f"Shop with ID {shop_id} not found for tenant '{tenant.name}'"
            )
        else:
            raise SeedingException(
                f"Shop with slug '{shop_slug}' not found for tenant '{tenant.name}'"
            )
...
def get_or_create_batch(model, defaults_list, unique_field, verbose=True):
    ...
    for item_data in defaults_list:
        ...
        obj, created = model.objects.get_or_create(
            **{unique_field: lookup_value}, defaults=item_data
        )
