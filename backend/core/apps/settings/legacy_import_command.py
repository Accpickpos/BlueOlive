...
class TenantAwareLegacyImportCommand(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--tenant', type=str, required=True, help='Tenant name or ID to import into')
        parser.add_argument('--shop', type=str, required=True, help='Shop name or code to import into')
        parser.add_argument('--dir', type=str, default=str(DEFAULT_PDF_DIR), help='Folder containing the legacy .dbf files')
        parser.add_argument('--migrate', action='store_true', help='Run migrations on the target schema before importing')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Tenant: {tenant.name}  Shop: {shop.name} (schema: {shop.schema_name})')
    ...
    def _get_tenant(self, identifier):
        try:
            ...
            return (
                Tenant.objects.filter(slug__iexact=identifier).first() or
                Tenant.objects.filter(name__iexact=identifier).first()
            )

    def _get_shop(self, tenant, identifier):
        return (
            tenant.shops.filter(code__iexact=identifier).first() or
            tenant.shops.filter(name__iexact=identifier).first()
        )
