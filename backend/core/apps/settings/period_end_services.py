...
class PeriodEndResult:
    ...
    def __init__(self, success: bool, message: str, details: Dict[str, Any] = None, errors: List[str] = None):
...
class DayEndService:
    ...
    def run_day_end(process_date: date = None, shop_id: int = None) -> PeriodEndResult:
        ...
        try:
            ...
            cashbook_summary = DayEndService._generate_cashbook_summary(process_date, shop_id)
    ...
    def _generate_stock_summary(process_date: date, shop_id: int = None) -> Dict:
        ...
        try:
            ...
            totals = queryset.aggregate(
                count=Count('id'),
                total_in=Sum('quantity_in') if hasattr(StockTransaction, 'quantity_in') else 0,
                total_out=Sum('quantity_out') if hasattr(StockTransaction, 'quantity_out') else 0
            )
            ...
            return {
                ...
                'data': {
                    'movement_count': totals['count'] or 0
                }
            }
        except Exception as e:
            return {
...
class MonthEndService:
    ...
    @staticmethod
    @transaction.atomic
    def run_month_end(process_date: date = None, advance_period: bool = True) -> PeriodEndResult:
        """
        ...
        try:
            ...
            if advance_period:
                period_advance = MonthEndService._advance_accounting_period(process_date)
    ...
    def _generate_department_stats(process_date: date) -> Dict:
        ...
        try:
            ...
            for dept in departments:
                ...
                profit_percent = (profit_value / sales_value * 100) if sales_value > 0 else Decimal('0')
            ...
            for area in areas:
                ...
                SalesAreaMonthlyStats.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'profit_value': profit_value,
                        'profit_percent': profit_percent,
                        'commission_earned': Decimal('0')  # Could be calculated from sales
                    }
                )
                created_count += 1
...
class YearEndService:
    ...
    @staticmethod
    @transaction.atomic
    def run_year_end(process_year: int = None, advance_year: bool = True) -> PeriodEndResult:
        """
    ...
    def _generate_year_end_summary(process_year: int) -> Dict:
        ...
        try:
            ...
            total_debtor_balance = Debtor.objects.aggregate(
                total=Sum('dcrnt') + Sum('d30') + Sum('d60') + Sum('d90') + 
                     Sum('d120') + Sum('d150') + Sum('d180')
            )
            
            # Creditor summary
            total_creditors = Creditor.objects.count()
            total_creditor_balance = Creditor.objects.aggregate(
                total=Sum('balance')
            )
            ...
            return {
                ...
                'data': {
                    ...
                    'total_creditor_balance': float(total_creditor_balance['total'] or 0)
                }
            }
        except Exception as e:
            return {
