    ...
    def analyze_csv(file_obj):
        ...
        try:
            ...
            return {
                ...
                'total_rows': sum(1 for line in text_content.splitlines()) - 1  # Subtract header
            }
...
class ImportViewSet(viewsets.ViewSet):
    ...
    def upload_and_analyze(self, request):
        ...
        if file_type == 'UNKNOWN':
            return Response(
                {'error': f'Unsupported file type. Use .csv or .dbf'},
        ...
        request.session[import_id] = {
            ...
            'mappings': mappings,
            'sample_data': [[str(v) if v is not None else '' for v in row] for row in analysis['sample_data']]
        }
    ...
    def preview(self, request):
        ...
        model_type = request.data.get('model_type', 'debtor')
        
        if import_id not in request.session:
            return Response({'error': 'Import session not found'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def import_data(self, request):
        ...
        if model_type == 'debtor':
            result = self._import_debtors(import_data, mappings, request.session, import_id)
        elif model_type == 'creditor':
            result = self._import_creditors(import_data, mappings, request.session, import_id)
        elif model_type == 'stock':
            result = self._import_stock(import_data, mappings, request.session, import_id)
        else:
            return Response({'error': f'Unknown model type: {model_type}'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for i, row in enumerate(rows):
            try:
                ...
                for django_field, source_col in mappings.items():
                    if source_col and source_col in headers:
                        ...
                        if col_idx < len(row):
                            value = row[col_idx]
                            # Convert value
                            data[django_field] = self._convert_value(value, django_field)
                ...
                debtor, created_new = Debtor.objects.update_or_create(
                    ...
                    defaults={k: v for k, v in data.items() if k != 'account_number' and v is not None}
                )
                ...
                progress.update(i + 1, total, f"Account: {data.get('account_number', 'Unknown')}")
    ...
    def _convert_value(value, field_name):
        ...
        try:
            # Boolean fields
            if 'blocked' in field_name or 'discount' in field_name.lower() and isinstance(value, str):
                if value.upper() in ('Y', 'YES', 'TRUE', '1'):
                    return True
                return False
            
            # Decimal fields
            if any(x in field_name for x in ['limit', 'balance', 'price', 'cost', 'amount', 'discount', 'markdown']):
    ...
    def progress(self, request):
        ...
        if not import_id:
            return Response({'error': 'import_id required'}, status=status.HTTP_400_BAD_REQUEST)
