...
class SalesDepartmentViewSet(BaseSettingsViewSet):
    ...
    def top_performers(self, request):
        ...
        metric = request.query_params.get(
            "metric", "sales_ytd"
        )  # sales_ytd, profit_ytd, sales_mtd, profit_mtd
        ...
        queryset = (
            self.get_queryset().filter(is_active=True).order_by(f"-{metric}")[:limit]
        )
        serializer = SalesDepartmentListSerializer(queryset, many=True)

        return Response(
            {"metric": metric, "limit": limit, "departments": serializer.data}
        )
...
class SalesAreaViewSet(BaseSettingsViewSet):
    ...
    def top_performers(self, request):
        ...
        return Response(
            {"metric": metric, "limit": limit, "sales_areas": serializer.data}
        )
...
class ExpenseCategoryViewSet(BaseSettingsViewSet):
    ...
    def by_type(self, request):
        ...
        queryset = self.get_queryset().filter(
            Q(category_type=category_type) | Q(category_type="BOTH"), is_active=True
        )
...
class SystemConfigurationViewSet(
    ...
    def advance_period(self, request):
        ...
        return Response(
            {"message": "Period advanced successfully", "config": serializer.data}
        )
    ...
    def run_day_end(self, request):
        ...
        if run_async:
            ...
            result = DayEndService.run_day_end(
                process_date=process_date, shop_id=shop_id
            )
            return Response(
                result.to_dict(),
                status=(
    ...
    def period_end_status(self, request):
        ...
        return Response(
            {
                "last_day_end_date": (
                    str(config.last_day_end_date) if config.last_day_end_date else None
                ),
                "last_month_end_date": (
                    str(config.last_month_end_date)
                    if config.last_month_end_date
                    else None
                ),
                "last_year_end_date": (
                    str(config.last_year_end_date)
                    if config.last_year_end_date
                    else None
                ),
                "scheduling": {
                    "day_end": {
                        ...
                        "time": (
                            str(config.day_end_time) if config.day_end_time else None
                        ),
                    ...
                    "month_end": {
                        ...
                        "time": (
                            str(config.month_end_time)
                            if config.month_end_time
                            else None
                        ),
                    },
                    "year_end": {
                        ...
                        "time": (
                            str(config.year_end_time) if config.year_end_time else None
                        ),
                    },
                },
                "current_period": {
...
class APIKeyViewSet(
    ...
    def get_queryset(self):
        ...
        if tenant:
            # Filter by tenant
            return APIKey.objects.filter(tenant=tenant).select_related(
                "tenant", "created_by"
            )
...
class EmailDocumentView(APIView):
    ...
    def post(self, request):
        ...
        subject = request.data.get(
            "subject", f"{document_type.title()} {document_number}"
        )
        ...
        sender_email = tenant.email or getattr(
