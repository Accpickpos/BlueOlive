...
for db_alias in ["default", "tenant_dev", "tenant_acme"]:
    try:
        ...
        # Check for tables
        cursor.execute(
            "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pos_jobcard'"
        )
        tables = cursor.fetchall()

        print(f"\n=== Database: {db_alias} ===")
        print(
            f"Settings: host={conn.settings_dict['HOST']}, db={conn.settings_dict['NAME']}"
        )
