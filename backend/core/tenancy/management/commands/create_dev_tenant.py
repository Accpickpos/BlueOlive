...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--name", type=str, default="Development Tenant", help="Tenant name"
        )
        parser.add_argument(
            "--slug",
            type=str,
        ...
        parser.add_argument(
            "--db-name", type=str, default="tenant_dev", help="Database name"
        )
        parser.add_argument(
            "--create-shop", action="store_true", help="Also create a default shop"
        )
        parser.add_argument(
            "--skip-existing",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if Tenant.objects.filter(slug=slug).exists():
            ...
            try:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Failed to create database: {str(db_error)}")
                    )
                    return
            ...
            if not skip_existing:
                ...
                try:
                    ...
                    self.stdout.write(
                        "No TTY detected, continuing with existing tenant."
                    )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"[OK] Tenant created: {tenant.name}")
                )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Failed to create tenant: {str(e)}")
                )
                return
        ...
        if create_shop:
            ...
            if Shop.objects.filter(tenant=tenant, code=shop_code).exists():
                ...
                try:
                    self.stdout.write(
                        f"Ensuring shop schema '{schema_name}' exists and is migrated..."
                    )
                    create_shop_schema(tenant, schema_name)
                    self.stdout.write(
                        self.style.SUCCESS("[OK] Shop schema verified and migrated")
                    )
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"[OK] Shop created: {shop.name}")
                    )
                    ...
                    self.stdout.write(
                        f"Creating shop schema '{schema_name}' and migrating shop apps..."
                    )
                    create_shop_schema(tenant, schema_name)
                    self.stdout.write(
                        self.style.SUCCESS("[OK] Shop schema created and migrated")
                    )

                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"Failed to create shop: {str(e)}")
                    )
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Failed to provision tenant database: {str(e)}")
                )
                return
        ...
        try:
            ...
            if (
                ...
                # Create admin user
                ShopUser.objects.using(tenant.db_alias).create(
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"[OK] Admin user created: {admin_username}")
                )
                self.stdout.write(
                    self.style.WARNING(
        ...
        if create_shop:
            self.stdout.write("  • http://127.0.0.1:8000/?shop=main")
