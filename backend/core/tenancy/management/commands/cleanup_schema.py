        ...
        for tenant in tenants:
            ...
            try:
                ...
                if not result['is_valid']:
                    self.stdout.write(self.style.WARNING(f"⚠️  Issues found:"))
                ...
                result = verify_schema_isolation(tenant)
                if result['is_valid']:
                    self.stdout.write(self.style.SUCCESS("✓ Schema isolation verified after cleanup"))
                else:
                    self.stderr.write(self.style.ERROR("⚠️  Still has issues after cleanup"))
                    for issue in result['issues']:
                        self.stderr.write(f"   - {issue}")

            except Exception as e:
                self.stderr.write(self.style.ERROR(f"✗ Error processing tenant: {str(e)}"))
                import traceback
                traceback.print_exc()
