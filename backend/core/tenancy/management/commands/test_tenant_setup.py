...
from tenancy.shop_manager import delete_shop_schema, get_schema_info
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--cleanup", action="store_true", help="Remove test tenant and data"
        )
        parser.add_argument(
            "--db-name",
            type=str,
    ...
    def create_test_tenant(self, name, db_name):
        ...
        tenant = Tenant.objects.create(
            ...
            db_port=5432,
            db_user="postgres",
            db_password="postgres",  # nosec B106 - local dev-only default, change for real use
            is_active=True,
        )
    ...
    def verify_tenant_database(self, tenant):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                cur.fetchone()[0]
                self.stdout.write("  ✓ Connected to PostgreSQL")
                ...
                self.stdout.write(
                    f"  ✓ Found {len(tables)} expected tables in public schema:"
                )
                for table in tables:
            ...
            self.stdout.write(
                self.style.ERROR(f"  ✗ Database verification failed: {str(e)}")
            )
            raise
    ...
    def verify_shop_schema(self, shop):
        ...
        self.stdout.write(self.style.SUCCESS("  ✓ Schema exists"))
    ...
    def print_summary(self, tenant, shops):
        ...
        self.stdout.write("      - auth, contenttypes")
        self.stdout.write("      - admin, token_blacklist")
        self.stdout.write("      - shop_users")

        for shop in shops:
            self.stdout.write(f"    {shop.schema_name}/")
            self.stdout.write("      - cash_book")
            self.stdout.write("      - creditors")
            self.stdout.write("      - stock_control")
            self.stdout.write("      - purchase_orders")
    ...
    def cleanup_test_data(self):
        ...
        for tenant in test_tenants:
            ...
            tenant.delete()
            self.stdout.write(self.style.SUCCESS("  ✓ Tenant record deleted"))
