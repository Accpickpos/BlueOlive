    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"  Failed to register connection: {e}")
                )
                continue

            try:
                shops = Shop.objects.using("default").filter(
                    tenant=tenant, is_active=True
                )
                for shop in shops:
                    self.stdout.write(
                        f"  Processing shop: {shop.name} ({shop.schema_name})"
                    )
                    ...
                    if fake_to_0001:
                        ...
                        with conn.cursor() as cur:
                            ...
                            self.stdout.write(
                                f"    Current creditors migrations: {current_migrations}"
                            )

                            if (
                                "0002_creditor_align_to_current_model"
                                in current_migrations
                            ):
                                # Fake back to 0001
                                cur.execute("""
                                ...
                                self.stdout.write(
                                    self.style.SUCCESS(
                                        "    [OK] Faked creditors back to 0001"
                                    )
                                )
                    ...
                    if re_run:
                        ...
                        try:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS("    [OK] Re-ran migrations")
                            )
                        except Exception as e:
                            self.stdout.write(
                                self.style.ERROR(f"    [X] Migration failed: {e}")
                            )
