...
class Command(BaseCommand):
            ...
            for shop in shops:
                ...
                try:
                    ...
                    if fixed:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"    Fixed tel2 column in {shop.schema_name}"
                            )
                        )
                    else:
                        self.stdout.write(
                            f"    tel2 already correct in {shop.schema_name}"
                        )
    ...
    def fix_tel2_column(self, tenant, shop, dry_run=False):
        ...
        if not conn:
            self.stdout.write(
                f"    Could not find connection for shop {shop.schema_name}"
            )
            return False
        ...
        with conn.cursor() as cursor:
            ...
            if not result:
                self.stdout.write(
                    f"    WARNING: tel2 column not found in {shop.schema_name}"
                )
                return False

            column_name, is_nullable, column_default = result
            self.stdout.write(
                f"    Current state: nullable={is_nullable}, default={column_default}"
            )
            ...
            if is_nullable == "NO" and column_default is None:
                self.stdout.write(
                    "    Column is NOT NULL but has no DEFAULT - this causes empty string to become NULL"
                )
                if not dry_run:
                    # Step 1: Drop NOT NULL constraint
                    cursor.execute("ALTER TABLE dmast ALTER COLUMN tel2 DROP NOT NULL")
                    ...
                    # Step 2: Set DEFAULT
                    cursor.execute("ALTER TABLE dmast ALTER COLUMN tel2 SET DEFAULT ''")
                    self.stdout.write("    Set DEFAULT empty string")
                    # Step 3: Re-add NOT NULL constraint
                    cursor.execute("ALTER TABLE dmast ALTER COLUMN tel2 SET NOT NULL")
                    self.stdout.write("    Re-added NOT NULL constraint")
                return True
            ...
            if null_count > 0:
                ...
                if not dry_run:
                    cursor.execute(
                        "UPDATE dmast SET tel2 = empty_string() WHERE tel2 IS NULL"
                    )
                    # Use empty string literal
                    cursor.execute("UPDATE dmast SET tel2 = '' WHERE tel2 IS NULL")
                    self.stdout.write(
                        f"    Updated {null_count} NULL values to empty string"
                    )
            ...
            if is_nullable == "YES":
                if not dry_run:
                    cursor.execute("ALTER TABLE dmast ALTER COLUMN tel2 SET NOT NULL")
                self.stdout.write("    Would add NOT NULL constraint")
                return True
