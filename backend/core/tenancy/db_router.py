...
class TenantDatabaseRouter:
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        ...
        if settings.DEBUG:
            logger.debug(
                f"[allow_migrate] db={db}, app_label={app_label}, model_name={model_name}"
            )
        ...
        if app_label in shop_app_labels:
            if db == "default":
                logger.warning(
                    f"[allow_migrate] BLOCK {app_label}.{model_name} → default DB (shop app)"
                )
                return False
            ...
            if not is_shop_migration:
                ...
                logger.warning(
                    f"[allow_migrate] BLOCK {app_label}.{model_name} → tenant public schema (SHOP_SCHEMA not set)"
                )
                return False

            logger.debug(
                f"[allow_migrate] ALLOW {app_label}.{model_name} → shop schema (SHOP_SCHEMA={shop_schema})"
            )
            return True
        ...
        if db != "default":
            logger.warning(
                f"[allow_migrate] BLOCK {app_label}.{model_name} → {db}. Only default DB for shared apps."
            )
    ...
    def _is_shop_schema_migration(self):
        ...
        if shop_schema:
            logger.warning(
                f"[_is_shop_schema_migration] SHOP_SCHEMA={shop_schema} (ACTIVE)"
            )
        elif tenant_db_alias:
            logger.warning(
                f"[_is_shop_schema_migration] TENANT_DB_ALIAS={tenant_db_alias} (tenant migration)"
            )
        else:
            logger.warning("[_is_shop_schema_migration] No migration context set")
