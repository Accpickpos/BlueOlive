...
class TenantAwareAuthBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        if settings.DEBUG:
            logger.debug(
                f"Authenticating {username} in tenant: {tenant.slug} (DB: {tenant.db_alias})"
            )
        ...
        try:
            ...
            if user.tenant_id != tenant.id:
                if settings.DEBUG:
                    logger.warning(
                        f"User {username} found but belongs to different tenant"
                    )
                return None
    ...
    def get_user(self, user_id):
        ...
        if not tenant:
            ...
            if getattr(settings, "ALLOW_DEFAULT_DB_FALLBACK", False):
                logger.warning(
                    "SECURITY: Using default DB fallback (should be disabled!)"
                )
                try:
