...
cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'acme_main' ORDER BY table_name")
...
cursor.execute("SELECT id, username, role, shop_ids FROM shop_users_shopuser ORDER BY id")
