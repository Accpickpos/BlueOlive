...
class ShopUserBackend(BaseBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        logger.info(
            f"Authenticate called for username: {username}, current tenant: {tenant.name if tenant else 'None'}, tenant db_alias: {tenant.db_alias if tenant else 'None'}"
        )
        ...
        try:
            ...
            if tenant:
                logger.info(
                    f"Querying user {username} in tenant database {tenant.db_alias}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Tenant database connection available: {tenant.db_alias}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to get tenant database connection: {tenant.db_alias}, error: {str(e)}"
                    )
                    raise
                ...
                with conn.cursor() as cur:
                    ...
                    if row:
                        ...
                        user.phone = row[13]
                        logger.info(
                            f"Found user in {tenant.db_alias}: {user.username}, checking password"
                        )
                    else:
            ...
            logger.debug(
                f"Found user: {username}, user tenant_id: {user.tenant_id}, current tenant id: {tenant.id if tenant else 'None'}"
            )
            ...
            logger.debug(
                f"Password valid: {password_valid}, can authenticate: {can_auth}"
            )
    ...
    def get_user(self, user_id):
        ...
        try:
            ...
            logger.debug(
                f"User {user.username} tenant_id: {user.tenant_id}, shop_id: {user.shop_id}"
            )
        except ShopUser.DoesNotExist:
        ...
        if user_tenant:
            tenant = get_current_tenant()
            logger.debug(
                f"get_user: user {user.username} (id={user_id}), user tenant: {user_tenant.name}, current tenant: {tenant.name if tenant else 'None'}"
            )
