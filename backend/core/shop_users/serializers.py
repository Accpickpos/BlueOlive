    ...
    def validate(self, attrs):
        ...
        if attrs.get("password") != attrs.get("confirm_password"):
            raise serializers.ValidationError(
                ...
                }  # nosec B105 - error message, not a credential
            )
    ...
    def create(self, validated_data):
        ...
        if not shop_ids:
            ...
            if first_shop:
                ...
                logger.info(
                    f"[UserCreationSerializer] Auto-assigned shop_id={first_shop.id} ({first_shop.name}) to new user"
                )
            else:
                logger.warning(
                    f"[UserCreationSerializer] No active shops found for tenant {tenant.name}!"
                )
