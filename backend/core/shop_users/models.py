...
class ShopUser(AbstractUser):
    ...
    shop_ids = models.JSONField(
        default=list, blank=True, help_text="List of shop IDs this user is assigned to"
    )
    ...
    email_verification_token_created = models.DateTimeField(
        null=True, blank=True, help_text="When the email verification token was created"
    )
    ...
    def verify_email_token(self, token, token_expiry_hours=24):
        ...
        if token_age > timedelta(hours=token_expiry_hours):
            ...
            return (
                False,
                f"Verification token expired (valid for {token_expiry_hours} hours)",
            )
    ...
    def get_active_shops(self):
        ...
        if not shop_ids_list:
            ...
            logger.warning(
                f"[get_active_shops] User {self.id} has NO shop_ids assigned!"
            )
            return Shop.objects.none()

        return Shop.objects.using("default").filter(
            id__in=shop_ids_list, is_active=True, tenant_id=self.tenant_id
        )
