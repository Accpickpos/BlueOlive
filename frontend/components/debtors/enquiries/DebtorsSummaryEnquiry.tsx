...
type ViewType = 'weekly' | 'monthly' | 'summary' | 'period' | 'inactive' | 'control';
...
export default function DebtorsSummaryEnquiry() {
  ...
  const loadDebtors = async () => {
    try {
      ...
      if (responseData.results) {
        setDebtors(responseData.results);
      } else if (Array.isArray(responseData)) {
        setDebtors(responseData);
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {viewType === 'control' && (
        <ControlEnquiry debtors={filteredDebtors} totals={totals} />
      )}
...
// Control Enquiry Component
function ControlEnquiry({
  debtors,
  totals,
}: {
  debtors: DebtorSummaryData[];
  totals: Record<string, number>;
}) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-900">Control Enquiry - Debtors Control</h3>

      <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
          <div className="border-r border-gray-200">
            <p className="text-xs text-gray-600 font-medium mb-2">Total Number of Debtors</p>
            <p className="text-3xl font-bold text-gray-900">{debtors.length}</p>
          </div>
          <div className="border-r border-gray-200">
            <p className="text-xs text-gray-600 font-medium mb-2">Total Amount Due</p>
            <p className="text-3xl font-bold text-red-600">R{Object.values(totals).reduce((a, b) => a + b, 0).toFixed(2)}</p>
          </div>
          <div>
            <p className="text-xs text-gray-600 font-medium mb-2">Average Per Debtor</p>
            <p className="text-3xl font-bold text-blue-600">
              R{(Object.values(totals).reduce((a, b) => a + b, 0) / Math.max(debtors.length, 1)).toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 pt-6 grid grid-cols-2 gap-4">
          <div>
            <p className="text-sm font-semibold text-gray-700 mb-3">Ageing Summary</p>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Current (0 days):</span>
                <span className="font-semibold text-gray-900">R{totals.current.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">30 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_30.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">60 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_60.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">90 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_90.toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
                <span className="text-gray-700 font-medium">Total:</span>
                <span className="font-bold text-gray-900">R{Object.values(totals).reduce((a, b) => a + b, 0).toFixed(2)}</span>
              </div>
            </div>
          </div>

          <div>
            <p className="text-sm font-semibold text-gray-700 mb-3">Aged Over 90 Days</p>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">90-120 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_120.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">150-180 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_150.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Over 180 days:</span>
                <span className="font-semibold text-gray-900">R{totals.age_180.toFixed(2)}</span>
              </div>
              <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
                <span className="text-gray-700 font-medium">Total 90+:</span>
                <span className="font-bold text-red-600">R{(totals.age_90 + totals.age_120 + totals.age_150 + totals.age_180).toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
