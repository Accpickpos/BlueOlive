...
export default function ReturnsPage() {
  ...
  const addLineItem = () => {
    ...
    setNewItem({ stock_code: '', quantity: 0, unit_cost: 0, reason: '' });
    setSearchResults([]);
  };
  ...
  const handleSave = () => {
    // In a real implementation, this would call the API to create a return transaction
    alert('Stock return transaction would be created via API');
    setShowForm(false);
    setLineItems([]);
    setFormData({ reference: '', supplier: '', return_date: new Date().toISOString().split('T')[0], reason: '', notes: '' });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Returns to suppliers</p>
        </div>
      </div>
      ...
      {!showForm && (
        <Card className="p-6">
          <h3 className="font-semibold mb-4">Recent Stock Returns</h3>
          <div className="text-center py-12">
            <ArrowUp className="w-16 h-16 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-600 text-lg">Stock returns would be listed here</p>
