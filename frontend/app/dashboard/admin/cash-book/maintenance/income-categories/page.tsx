...
export default function IncomeCategoriesPage() {
  ...
  const validateCategoryCode = (numberStr: string): boolean => {
    // Backend range: IncomeCategory.number is a PositiveIntegerField,
    // 1-99999999 (apps/settings/models.py) — free-form, not restricted to
    // a Chart-of-Accounts band. The manual's own example categories
    // (8500, 3020, 1300...) span ranges a fixed band would reject.
    const numRegex = /^[0-9]{1,8}$/;
    if (!numRegex.test(numberStr)) {
      setValidationError('Category number must be numeric, 1-8 digits (e.g., 4100)');
      return false;
    }
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage income category numbers (1-99999999)</p>
        </div>
