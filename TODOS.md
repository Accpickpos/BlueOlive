...
## POS: Subtotal Discount facility (Invoice/JobCard)

**What:** A single discount % entered once at update-time, applied
proportionally across every line item on the transaction — documented for
both Invoice (manual §1 [11.htm], "Subtotal Discount Facility") and Job
Costing (§1 [113.htm], via the Update Transaction "S" option).

**Why:** Found auditing `apps/pos` against `PointOfSale.pdf`/the reconstructed
manual (2026 module-by-module behavior audit). No trace of this facility
anywhere in `apps/pos` (models, views, services) — not a partial
implementation, entirely absent.

**Pros:** Real documented workflow — a legitimate gap, not cosmetic.

**Cons:** Needs a design decision before implementation: how does a
transaction-level discount interact with per-line `discount_percentage` and
`maximum_discount_percent` caps that already exist (`InvoiceLine`,
`CashSaleLine`)? Does it compound with an existing line discount, replace it,
or get rejected if any line would exceed its own max-discount cap once the
subtotal discount is layered on?

**Context:** POS module audit, `prompt.txt` Phase 2, module 1 of 9.

**Depends on / blocked by:** None — self-contained to Invoice/JobCard line
pricing.

---

## POS: Set Selling Price facility (Invoice/JobCard)

**What:** Enter one target inclusive total for the whole transaction; the
system back-solves and proportionally adjusts every line's unit price to
hit that total exactly. Documented for both Invoice and Job Costing
(manual §1 [113.htm]: "Set Price... revised inclusive Total Amount... system
automatically adjusts the prices of individual items in proportion").

**Why:** Same audit as above. No trace anywhere in `apps/pos`.

**Pros:** Real documented workflow, same as Subtotal Discount.

**Cons:** Needs a design decision on rounding (proportional adjustment across
N lines rarely divides evenly to the cent — which line absorbs the
remainder?) and on edge cases (zero-price lines, "no value" comment lines
per the manual — these shouldn't be scaled).
...
## POS: Cash-Debtor zero-terms auto-tender on Invoice

**What:** Manual (§1 [11.htm], "Cash Debtors" note): "Where the Debtor's
Account Category is set to C in Debtor's File Maintenance and their payment
terms are set to 0, the invoice transaction will end with a Tender Routine,
as in a Cash Sale. An invoice plus a payment will be posted to the Cash
Debtor's Account." I.e. for these specific debtors, completing an Invoice
should immediately prompt for payment and post both the invoice and its
payment together, rather than leaving it as a normal open receivable.

**Why:** Same audit. `Debtor.acctype` ('C' = Cash Customer) and `Debtor.terms`
already exist (`apps/debtors/models.py`) but nothing in `apps/pos` checks
this combination or triggers a tender step — confirmed via grep, no hits for
this behavior anywhere in the app.

**Pros:** A real, specific, testable business rule directly from the manual —
not a vague feature request.

**Cons:** Cross-cuts `Debtor`, `Invoice`, `Tender`, and `CashControl` — a
workflow design decision (does the API auto-create a `Tender`/payment record
server-side on Invoice completion when this condition is met, or does the
frontend need to detect the condition and prompt the cashier before calling
a separate payment endpoint?), not a local model/serializer fix.

**Context:** POS module audit, `prompt.txt` Phase 2, module 1 of 9.

**Depends on / blocked by:** None, but should be designed alongside the
Receipts-on-Account settlement-discount item below since both touch
Invoice/Debtor payment completion flows.

---

## POS: Generic transaction reverse/void (all transaction types)

**What:** A consistent way to reverse/void any posted POS transaction type,
not just Invoices.

**Why:** Same audit. Only `Invoice` has `cancel`/`void` actions
(`apps/pos/views.py`) — `CashSale`, `CreditNote`, `CashReturn`,
`ReceiptOnAccount`, `CashACheque` have no reversal path at all once posted.
The manual implies most transaction types are correctable/reversible
through the DOS UI's own mechanisms.

**Pros:** Closes an operational gap — currently a mis-keyed Cash Sale or
Receipt has no undo path in the app.

**Cons:** Real design question, not a local fix: which types are actually
reversible (a posted `CashSale` already moved stock and till totals —
reversal needs to unwind both, matching `CashSaleService.cancel_sale`'s
existing pattern), and what are the GL/stock implications per type. Needs a
decision on whether this becomes one generic mechanism or type-specific
`cancel`/`void` actions added individually (`CashSaleService.cancel_sale`
already exists and could be the template).

**Context:** POS module audit, `prompt.txt` Phase 2, module 1 of 9.

**Depends on / blocked by:** None, but the General Ledger module audit
(module 7) should inform the "what GL implication per type" part of this
before it's implemented.

---

## POS: Full TransactionQuery search across all transaction types

**What:** `TransactionQuery.search` currently only searches
CASH_SALE/LAYBYE/QUOTATION; the manual's Transaction Query facility implies
broader lookup capability across all POS transaction types (Invoice,
Receipt on Account, Credit Note, Cash Return, Repair, Job Card, Payout,
Cheque).

**Why:** Same audit — confirmed via the earlier `apps/pos` survey
(views.py's `search` action).

**Pros:** Straightforward extension of an existing, working pattern — lower
design risk than the other items on this list.

**Cons:** Not zero-risk: each additional transaction type needs its own
search-field mapping (what does "search" mean for a Repair vs. a Cheque?),
and the result shape needs to stay generic enough for one UI to render.
Logged rather than done inline because it touches every transaction model
in the app, not a single local file.
...
## POS: "Charge for the Repair" (Cash/Account invoicing flow)

**What:** Manual (§1 [111.htm], "5. Charge for the Repair"): a genuine
two-way branch, not a status-flip. **Cash** option: creates a Cash Sale
transaction, ends with a Tender Routine. **Account** option: choose an
existing Debtor or capture a new one on the fly, then creates an Invoice.
In *both* branches, line items are manually (re-)captured at charge time
(with a cost-price toggle: "Caps locks on and press C") — they are **not**
auto-copied from the `RepairLine` issue/receipt records, which are a
separate supplier-side bookkeeping trail (issue-to-supplier,
receive-from-supplier, adjustment types).

**Why:** `RepairService.invoice_customer` (`apps/pos/services.py`) is
currently a stub that only sets `status = 'V'` — doesn't create any
Cash Sale or Invoice at all. Originally scoped as a "small fix following
the same conversion pattern as `convert_quotation_to_invoice`" in this
audit's plan, but reading the actual manual section revealed the real
behavior is structurally different (branching charge type + manual line
re-capture, not a line-for-line copy) — logged instead of implementing the
wrong thing.

**Pros:** Closes a real, currently-nonfunctional workflow (repairs can never
actually be invoiced to a customer today).

**Cons:** `Repair` has no `debtor` FK today (only free-text
customer_name/address/telephone, unlike Quotation/JobCard/Laybye) — "Account
→ New Debtor" needs debtor-creation-on-the-fly, a real design question. Needs
a decision on API shape: does charging a repair reuse the existing
`CashSaleService`/`Invoice` creation endpoints with repair context attached,
or does it need a dedicated `charge_repair` endpoint that pre-fills a line
from `repair.selling_price`/`repair_cost` for the cashier to confirm/edit?

**Context:** POS module audit, `prompt.txt` Phase 2, module 1 of 9.

**Depends on / blocked by:** None, but should reuse whatever the
Cash-Debtor-auto-tender item above lands on for the Cash-branch tender step.

---

## POS: Receipts on Account — settlement discount is structurally incomplete

**What:** The manual documents two distinct settlement-discount flows,
neither of which matches the current `ReceiptOnAccount` model:
- **Balance Brought Forward debtors:** enter amount due + amount tendered;
  the system computes the settlement discount *amount* as the difference
  (`due - tendered`), then derives and displays the discount *percentage*
  from that amount — the opposite direction from a %-to-amount calculation.
- **Open Item debtors:** payment is allocated line-by-line against specific
  open transactions (arrow-key allocation screen), and settlement discount
  is only permitted when an item is paid in **full** — "No Settlement
  Discount on part payments" is an explicit documented rule.

**Why:** Originally scoped as "wire `ReceiptOnAccount` to the existing-but-
unused `CalculationService.apply_settlement_discount`" in this audit's plan.
Reading the actual manual section (§1 [12.htm]) showed that function computes
the wrong direction (% → amount; the manual needs amount-difference → %) and
that `ReceiptOnAccount` (`apps/pos/models.py`) has no per-transaction
allocation structure at all — it's a flat amount/settlement_discount/
total_amount record, with no link to `debtors.DebtorOpenItem`. This is a
structural gap, not a wiring gap — logged instead of a superficial fix that
would leave the real Open Item behavior unimplemented.

**Pros:** Closes a real gap — Open Item debtor payment allocation (with its
full-payment-only discount rule) doesn't exist in the app today.

**Cons:** Needs a new allocation-line model (linking a receipt to one or more
`DebtorOpenItem` records, each with its own paid amount, mirroring
`creditors.OpenItemAllocation`'s existing pattern on the creditor side) —
real schema work, not a local fix. Also needs a decision on whether
`Debtor.acctype` (BBF vs Open Item, per `apps/debtors/models.py`) should
drive which UI/API flow is offered, matching the manual's own branching.

**Context:** POS module audit, `prompt.txt` Phase 2, module 1 of 9. **Update
from the Debtors module audit (module 2 of 9):** `apps/debtors` has its own,
separate, equally-incomplete receipt-posting path
(`DebtorService.post_receipt` / `DebtorTransactionViewSet.post_receipt`,
`apps/debtors/services.py:184-218`) — a flat `RCP` posting with no
settlement-discount logic at all. The manual describes this exact facility
(Batch Receipt Posting, BBF and Open Item) as reachable from both the
Debtors menu and the POS menu, so this is one underlying gap, not two — a
real fix needs to land in one place both callers use, not be patched twice
independently. Confirmed manual behavior for Open Item debtors is line-by-
line allocation via `Debtopen` records, which now (as of this audit) are
correctly created for Open Item debtors — see the `post_debtran` open-item
condition fix below.

**Depends on / blocked by:** None further — the Debtors-side investigation
this was originally blocked on is now done (see above).
